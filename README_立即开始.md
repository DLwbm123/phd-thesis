...
本版本依据 2026-07-31 导师讨论后的论文框架更新。开始继续写第二章之前，必须先完成一次框架迁移。
...
当前仓库第一章和章节卡仍基于旧结构。先将本迁移包放到仓库外或仓库中的临时目录，然后在 Codex Sol High 中执行：

```text
请读取并严格执行 tasks/CODEX_TASK_导师框架迁移_2026-07-31.md。
不得丢弃任何未提交修改。先建立可恢复分支或备份，再完成目录、约束文件、第一章和证据映射迁移。
```
