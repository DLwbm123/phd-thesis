...
第三章建立了对知识保持、当前任务学习、前向泛化和资源条件分别评价的共同框架；第四章和第五章分别在集中式有限回放条件下研究持续配准与弱监督持续分割。本章进一步收紧信息访问范围：不同医疗机构的原始影像不能集中，客户端学习新分类任务时也不重新调用历史原始样本。此时，知识保护既要跨越时间，也要覆盖其他客户端曾经学习、而当前客户端无法直接访问的任务，单个学习器依赖历史样本回放的方案不再适用。本文据此研究无回放联邦持续医学图像分类（replay-free federated continual learning，replay-free FCL）\cite{mcmahan2017fedavg,rieke2020future,yoon2021fedweit}。

在这一设定下，客户端数据的非独立同分布使遗忘不再局限于单个机构。本章将全局灾难性遗忘定义为：随着新任务到达，服务器端全局模型在联邦范围内已学习任务上的性能下降。局部遗忘只描述某个客户端自身任务历史的知识损失；全局遗忘还包括其他机构曾经贡献、但当前客户端无法直接访问的历史知识。各客户端只使用当前本地数据更新模型时，类别比例、样本数量、扫描设备和人群构成等差异会使局部更新偏离联邦总体目标，聚合这些更新后，旧任务能力仍可能下降\cite{dong2022glfc}。

在无回放约束下，正则化和梯度投影可以在不回放旧图像的条件下减轻遗忘。梯度投影记忆通过保存旧任务的重要方向，将新任务梯度投影到其正交补空间\cite{saha2021gpm}。然而，原始方法面向观察完整任务序列的单一学习器。在联邦环境中，客户端独立构建的保护子空间只覆盖自身历史，不能直接包含仅在其他机构出现的任务方向；若服务器无差别叠加全部客户端方向，可用于学习当前任务的更新空间又会随之收缩。因此，本章需要同时解决跨客户端历史覆盖和本地可塑性保留两项问题。

基于上述矛盾，本章提出联邦子空间融合方法 FedSubMerge。每个客户端在当前任务结束时利用样本级梯度与服务器此前返回的历史摘要，递归构建低秩主梯度子空间。服务器对客户端上传的子空间基和奇异值进行融合，形成覆盖跨客户端历史方向的共享保护子空间；后续任务训练时，客户端将共享特征提取器的梯度投影到该子空间的正交补，以降低更新对旧任务损失的一阶干扰。在此基础上，FedSubMerge-AD 按网络层计算客户端子空间距离，为每个目标客户端选择相关客户端并构建个性化保护子空间，从而减少不必要的方向约束。

本章建立任务模型、方法与理论分析，并在病理图像、腹部 CT、消化内镜和多来源皮肤镜数据上评价分类性能、知识保持及通信开销。最后给出联邦分类模块与第三章规划的综合平台之间的接口：客户端训练、服务器融合、阶段评价和持久化对象属于已有实验流程，真实多服务器通信、身份认证、异常恢复和统一界面仍需在平台完成后验证。低秩梯度子空间交换只说明历史原始图像未被直接回放或集中，不构成形式化隐私保证。
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{ch06/fedsubmerge_overview.pdf}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.94\textwidth]{ch06/fedsubmerge_hyperparameter.pdf}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.94\textwidth]{ch06/fedsubmerge_accuracy_matrix.pdf}
...
\section{联邦分类模块的工程实现与平台接口}
\label{sec:fedsubmerge-engineering}

\subsection{算法模块与平台功能的对应关系}
...
\begin{figure}[htbp]
\centering
\input{figures/ch06/fedsubmerge_engineering.tex}
...
\subsection{综合平台的实验管理接口与部署边界}

面向第三章所规划综合平台的集成应优先统一实验管理，而不是强行合并不同任务的算法。共同管理项可以包括病例划分版本、任务顺序、监督类型、历史访问权限、模型版本和逐阶段评价结果。分割仍保留像素标签与 Dice 等指标，配准保留图像对、变换和 TRE 等指标，分类保留客户端标签空间与准确率。这样可以在同一管理流程中追踪“用什么数据、更新哪个模型、保留哪些历史能力”，同时保留各任务的独立评价标准。
