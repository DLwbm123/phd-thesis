...
ZSDERpp & 与 ZS-Sequential 相同 & 64 幅图像；水库采样；回放批大小为 8；保存涂鸦与参考特征 & 域增量与跨器官实验 \\
ZSDERpp + MiB & 涂鸦监督与背景语义适配 & 有限回放、参考特征与背景语义保持 & 类别增量实验 \\
\bottomrule
\end{tabularx}
\end{table}
...
\caption{ScribbleCL 三类持续分割场景的涂鸦标注与任务序列。每一行对应一种持续分割设定，箭头表示任务到达顺序；蓝色曲线表示背景涂鸦，其他彩色曲线表示当前阶段可访问的前景类别涂鸦，未被涂鸦覆盖的像素在弱监督训练中设为忽略位置。图中“器官增量”对应本文的跨器官任务增量。Domain-CL、Class-CL 和 Organ-CL 分别对应本章的六中心、三任务类别增量和四任务跨器官实验。}
...
在三任务类别增量实验中，ZSDERpp 与 MiB 结合后的 A-Dice、BWTR、RMA 和 WCD 分别为 $0.7677$、$0.0031$、$0.9845$ 和 $0.7067$。其中，A-Dice、BWTR 和 WCD 均为表中最高值。BWTR 为接近零的正值，表示旧任务从初学到最终阶段的平均相对表现略有提高，但不意味着每个旧任务都没有遗忘；RMA 接近 1，表明新增任务的初学表现平均接近独立训练参照。

相较 ZS-Sequential，ZSDERpp 与 MiB 组合的 A-Dice 提高 $0.3067$，BWTR 提高 $0.6451$，WCD 提高 $0.4004$，而 RMA 低 $0.0593$。相较 Dense-Sequential，其 A-Dice 和 WCD 分别提高 $0.2833$ 和 $0.3758$，BWTR 从 $-0.6785$ 提高到 $0.0031$；Dense-Sequential 的 RMA 为 $1.1391$，仍是表中最高值。因此，组合方法的主要优势体现在旧类别保持与最终统一输出，而不是所有评价指标都最优。该比较同时改变了监督形式和历史信息使用，不能将结果差异仅归因于涂鸦或某一个损失项。

类别增量中的 ZS-ER 和 ZS-DER 分别取得 $0.3616$ 和 $0.4141$ 的 A-Dice，均低于 ZS-Sequential 的 $0.4610$；两者 BWTR 分别为 $-0.5948$ 和 $-0.5875$，虽高于 ZS-Sequential 的 $-0.6420$，仍伴随明显历史退化。有限回放本身因此不能保证最终全类别表现改善。ZSDERpp 与 MiB 组合利用历史涂鸦和参考特征保留已有结构，并处理旧类别与新增类别之间的背景语义变化；当前结果支持这一完整方案的效果，但尚不能单独量化背景适配与特征约束各自的贡献。
...
资源方面，Class-CL 表中各方法的 MPE 均为 $0.0000$，不依赖模型参数扩展。PCE-Sequential、Dense-Sequential、ZS-Sequential 和 ZS-EWC 的 DRR 均为 $0.0000$；ZS-GPM 的 DRR 为 $0.0107$，对应历史子空间信息；ZSDERpp 与 MiB 组合、ZS-ER 和 ZS-DER 的 DRR 分别为 $0.0313$、$0.0333$ 和 $0.0317$，均涉及历史原始图像回放。前者在较接近的数据使用比例下取得更高 A-Dice 与 WCD，但 DRR 不能替代回放对象的字节开销和附加计算统计。
...
域增量保持前景语义不变，重点考察中心适应和旧域保持；类别增量持续扩展心脏结构，需要同时处理已见类别与阶段背景语义；跨器官任务增量改变目标器官、模态与任务输出语义。ZSDERpp 与 MiB 的组合在类别增量中保留背景概率适配，Organ-CL 则使用已知任务对应的二通道输出头。A-Dice、BWTR、RMA 以及适用场景下的 E-FWT 或 WCD 分别反映不同能力，不能由单一结果替代。
