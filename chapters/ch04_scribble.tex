...
本章沿用第三章 MedCL 的任务设定和阶段评价，研究持续医学图像分割。首先构建密集监督基准，比较中心变化、类别扩展和跨器官任务变化下的方法表现；随后保留域增量与类别增量的病例划分和测试参照，将训练标签改为固定模拟涂鸦，研究标注不足时如何学习新任务并保持旧任务能力。两部分均以 Dice 构建阶段--任务矩阵，使用 BWTR、RMA 和资源指标，并按场景补充 E-FWT 或 WCD。
...
本章先说明基准设计与密集监督评测结果，再介绍弱监督问题和 ZSDERpp 方法，最后通过六中心域增量与三阶段类别增量实验，分析监督增强、历史保持和资源使用之间的关系。

\section{持续医学图像分割的问题定义与基准设计}
\label{sec:benchmark-instance}

本节按第三章的场景定义构建持续分割基准，规定密集监督下的任务、数据划分、输出方式和训练配置。基准覆盖域增量、类别增量与跨器官任务增量，用于比较不同知识保持方法，也为后续弱监督实验提供固定数据与测试参照。图\ref{fig:benchmark-overview}给出整体构成；Organ-CL 沿用原跨器官序列的标识，在本文中属于任务增量实例。
...
持续语义分割综述讨论了稳定性--可塑性冲突、语义漂移和背景变化等问题\cite{yuan2024continualseg}，但多数实验建立在 PASCAL VOC、ADE20K 等自然图像数据上。医学影像中的跨中心变化可能在标签空间不变时显著改变图像外观；类别增量分割受到解剖结构关系和空间先验影响；跨器官任务又会同时改变模态、结构和输出语义。这些条件使自然图像持续分割的结论不能直接等同于医学场景。
...
测试方式还需结合输出空间解释。Domain-CL 保持标签语义不变，使用共享输出头；Class-CL 逐步扩展统一标签空间，并要求最终模型预测全部已见类别；Organ-CL 的任务目标、成像模态和输出语义均可改变，因而采用任务特定输出头。本章采用预先给定阶段划分和任务切换时机的训练方式，并按上述输出头完成测试。本章不研究测试时的任务身份推断、任务切换检测或开放世界识别。
...
类别增量分割不仅需要逐步学习新增类别，还受到背景语义变化影响。当前阶段只对新增类别提供监督时，旧类别或未来类别像素可能被并入背景，导致“背景”在不同阶段不再表示相同语义。普通知识蒸馏或参数约束若忽略这一变化，可能把当前新增结构误当作旧背景，或使旧类别在后续阶段被覆盖。基准因此引入 MiB 和 PLOP 等面向持续语义分割的专项方法作为对照\cite{cermelli2020mib,douillard2021plop}。
...
本章选择正则化、回放和参数隔离三类具有代表性的通用持续学习策略，并在 Class-CL 中增加面向持续语义分割的专项方法。选择这些方法的目的不是穷尽所有持续学习算法，而是在统一网络和训练预算下覆盖不同的历史知识保存方式：参数重要性、旧模型输出、原始样本、梯度约束、低维子空间以及任务特定参数。
...
持续分割还可能同时面对多中心数据分散、测试分布变化或更少标注。图\ref{fig:benchmark-related-paradigms}概括了这些问题与其他学习方法的联系。以下讨论在密集监督、已知阶段划分的基准之外，还需要考虑哪些训练与评价条件；图中列出的方向并非都已在本章完成实验。

联邦学习处理不同中心的数据不能直接集中共享的问题，持续学习则处理同一中心内部数据或任务随时间变化的问题。二者结合后，模型需要同时面对空间异质性和时间演化，并在不交换原始数据的条件下保留跨客户端历史知识\cite{yang2024fclsurvey,yoon2021fedweit}。传统同步联邦训练还可能受到不同中心任务进度和计算条件不一致的影响。第三章提供共用的场景定义与评价指标，本章基准进一步显示历史保持、当前学习与资源使用之间的取舍，为第五章分析无回放联邦分类中的知识保护提供参照。

测试时适应则利用无标签目标数据在部署阶段更新模型，不要求重新访问源训练集\cite{wang2021tent,hu2021fullytta}。这一条件与临床中“新中心数据已经到达，但像素级标注尚未获得”的情况相符。若目标分布继续变化，模型还需要在适应当前测试流的同时保持此前测试域和源域能力，从而形成持续测试时适应问题。本章发现多数方法的前向泛化提升有限，说明仅依赖训练阶段的历史保护不足以保证模型在未知中心直接有效；前向泛化评价、分布变化检测和无标签在线适应需要进一步结合。
...
在此基础上，本章固定域增量和类别增量的数据划分与测试参照，将训练标签改为模拟涂鸦，提出融合监督增强、有限图像--涂鸦回放与参考特征约束的 ZSDERpp。六中心实验中，其 A-Dice 为 $0.7687$、BWTR 为 $-0.1520$，优于增强涂鸦顺序训练的 $0.5100$ 和 $-0.3240$。三任务类别增量中，与 MiB 结合后的 A-Dice、BWTR、RMA 和 WCD 分别为 $0.7793$、$-0.0034$、$0.9892$ 和 $0.7292$。这些结果说明，在固定模拟涂鸦条件下，监督增强与有限历史信息能够共同改善持续分割。下一章研究不回放历史原始样本的联邦持续分类。
