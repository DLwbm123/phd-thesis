...
\begin{figure}[p]
\centering
\includegraphics[width=\textwidth,height=0.80\textheight,keepaspectratio]{ch04/benchmark_scenarios_large.pdf}
\caption[持续医学图像分割基准的整体构成]{持续医学图像分割基准的整体构成：持续学习场景。域增量、类别增量和跨器官任务增量分别对应中心变化、标签扩展及分割对象变化。原图中的 Organ-CL 对应本文的跨器官任务增量实例；统一训练设置与多维评价见续图。}
...
\centering
\includegraphics[width=\textwidth,height=0.80\textheight,keepaspectratio]{ch04/benchmark_training_evaluation_large.pdf}
\caption{持续医学图像分割基准的整体构成（续）：统一顺序训练设置与多维评价。左侧为统一任务定义、数据划分和各类方法，右侧为任务性能、持续学习能力、资源指标及主要发现。}
\end{figure}
...
\phantomsection\label{subsec:benchmark-domain-cl}
域增量场景（Domain-CL）描述分割目标保持不变，而图像分布随中心、设备或采集参数发生变化的情况。对任意两个任务 $t_1$ 和 $t_2$，其标签集合满足 $Y_{t_1}=Y_{t_2}$，但输入分布通常满足
...
\phantomsection\label{subsec:benchmark-class-cl}
类别增量场景（Class-CL）保持成像区域和图像分布相对稳定，但分割类别随阶段扩展。设第 $t$ 阶段提供标签集合 $Y_t$，则不同阶段满足 $Y_{t_1}\neq Y_{t_2}$，最终模型需要在统一输出空间中预测全部已见类别。本章使用心脏 CT 数据，将七个解剖结构依次组织为三个阶段：第一阶段学习左心室、左心房和心肌，第二阶段加入右心室和右心房，第三阶段加入升主动脉和肺动脉。
...
\phantomsection\label{subsec:benchmark-organ-cl}
跨器官任务增量场景（Organ-CL）进一步改变分割对象、图像分布和成像模态。令 $Y_t^{+}=Y_t\setminus\{0\}$ 表示不含背景的前景类别集合。不同任务的前景类别互不重叠，即
...
在数据划分方面，三类场景中的每个分割任务均在病例层面随机划分为 60\% 训练集、15\% 验证集和 25\% 测试集，验证集用于方法超参数选择。不同持续学习方法使用相同的任务定义和数据划分，避免因训练样本或验证样本不一致影响比较。
...
\begin{figure}[p]
\centering
\includegraphics[width=\linewidth,height=0.75\textwidth,keepaspectratio]{ch04/task_robustness_adice.pdf}
...
\begin{figure}[p]
\centering
\includegraphics[width=\linewidth,height=0.75\textwidth,keepaspectratio]{ch04/task_robustness_bwt_from_bottom.pdf}
...
\begin{figure}[p]
\centering
\includegraphics[width=\linewidth,height=0.75\textwidth,keepaspectratio]{ch04/domain_memory_size_adice.pdf}
...
前述实验表明，现有持续学习方法在 Domain-CL 中的前向泛化仍然有限。由此产生一个直接问题：大规模预训练获得的通用表征，能否在中心域连续到达时同时改善适应、保持和未来域泛化。为对此进行初步观察，本章进一步在六中心 Domain-CL 上考察 Segment Anything Model（SAM）\cite{kirillov2023sam} 的两种顺序适配方式。需要说明的是，此处的 SAM 指基础分割模型 Segment Anything Model，与第六章持续配准方法中使用的锐度感知最小化并非同一概念。
...
\begin{figure}[p]
\centering
\includegraphics[width=\linewidth,height=0.75\textwidth,keepaspectratio]{ch04/SAM_confusion_matrix.pdf}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{ch04/plots_for_benchmark.pdf}
...
在评价参照方面，六中心实验采用 NCI-ISBI 2013、I2CVB 和 PROMISE12 随影像提供的前列腺密集分割标注作为评价参照\cite{dataset_prostate2,dataset_prostate,dataset_prostate3}；类别增量实验采用 MMWHS 的心脏结构标注，并沿用本章基准的七类结构与背景编码\cite{care25ASA1}。
...
\begin{figure}[p]
\centering
\includegraphics[width=\linewidth,height=0.75\textwidth,keepaspectratio]{ch04/scribble_annotation_fig.pdf}
