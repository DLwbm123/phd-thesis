...
第三章的 MedCL 平台以任务序列和阶段--任务性能矩阵组织持续学习评价，并通过分割基准考察中心变化、类别扩展和跨器官任务变化。本章沿用其中的六中心前列腺 Domain-CL 与三任务心脏结构 Class-CL 设置，将训练监督由密集掩膜改为固定模拟涂鸦，研究标注信息受限时的新任务学习与历史知识保持。实验继续采用平台的 A-Dice、BWTR、RMA、MPE 和 DRR，并分别使用 E-FWT 与 WCD 评价未来域泛化和全类别分割表现。

密集医学图像分割标注需要逐像素或逐体素描述器官与病灶，新的医疗中心和分割类别持续出现时，为每个阶段获得完整掩膜会形成较高监督负担。涂鸦标注只在少量位置给出类别信息，部分交叉熵只约束已标注位置，未标注区域仍需由图像信息、一致性关系和结构先验共同推断\cite{tajbakhsh2020imperfect,can2018scribble,zhang2026zscribbleseg}。中心域变化还要求模型适应新的图像分布，类别扩展则改变当前可访问的标签与背景语义；后续更新均可能降低模型已经获得的分割能力。

本章沿用第三章相应基准的病例划分、任务顺序、预处理、输出组织和密集测试参照。训练集密集掩膜离线生成固定模拟涂鸦，弱监督训练读取涂鸦及忽略位置；验证集和测试集保留密集标签，分别用于模型选择与评价。由此，平台中的域增量和类别增量设定进一步与弱监督、有限历史回放条件结合。

前期 ZScribbleSeg 研究通过全局一致性约束未标注区域，并利用候选非目标区域抑制分割区域的不合理扩张\cite{zhang2026zscribbleseg}。本章将涂鸦监督下的持续分割问题记为 ScribbleCL，提出 ZSDERpp，将当前任务监督增强、有限历史图像--涂鸦回放和参考特征约束结合。面向类别扩展，进一步与 MiB 的背景语义处理结合，使旧类别保持与新增类别学习使用相容的输出表示。

本章首先定义弱监督持续分割问题，随后介绍监督增强、特征一致性约束及输出适配，在六中心域增量和三任务类别增量序列上分析结果，最后说明上述数据、阶段预测和指标在 MedCL 评测框架中的对应关系。
...
在历史访问方面，本章不允许后续阶段完整读取全部旧任务训练集，但 ZSDERpp 可以从容量受限的回放集合读取少量历史图像--涂鸦对及其参考特征。当前任务的监督增强、历史样本的弱监督优化和参考特征约束共同参与更新。类别增量设置进一步令已见类别集合随阶段扩展，使用共享八通道输出头，并按照第三章的阶段标签设定处理背景。其阶段--任务矩阵仍以密集测试标签计算，最终阶段同时评价全类别输出。
...
六中心域增量实验复用第\ref{chap:benchmark}章表~\ref{tab:benchmark-datasets}中的前列腺病例、中心顺序和病例级划分，三任务类别增量实验沿用同章 MMWHS 心脏 CT 的类别分组\cite{wang2026benchmark}。数据按 60\% 训练、15\% 验证和 25\% 测试组织，不重新抽样；相同病例的切片归入同一子集。两类实验均将训练监督改为固定模拟涂鸦，验证和测试参照保持不变。表\ref{tab:scribblecl-datasets}列出对应数据与输出配置，同时给出跨器官任务的输出组织。
...
六中心实验对 C 中心执行裁剪和空间对齐，将所有轴位切片调整为 $256\times256$，并使用非零体素的均值和标准差归一化。类别增量实验采用第三章 MMWHS 的预处理、类别映射与三阶段划分。病例划分、任务顺序和测试标签均按相应 MedCL 基准设置组织。

\subsubsection{参考标注及其用途}
\label{subsubsec:scribblecl-reference-standard}

六中心实验采用 NCI-ISBI 2013、I2CVB 和 PROMISE12 随影像提供的前列腺密集分割标注作为评价参照\cite{dataset_prostate2,dataset_prostate,dataset_prostate3}；类别增量实验采用 MMWHS 的心脏结构标注，并沿用第三章的七类结构与背景编码\cite{care25ASA1}。
...
表\ref{tab:scribblecl-domain-results}给出六中心前列腺 MRI 序列上的 Domain-CL 结果，表\ref{tab:scribblecl-class-results}给出三任务心脏结构序列上的 Class-CL 结果。两类实验共同报告 A-Dice、BWTR、RMA、MPE 和 DRR，分别从最终性能、历史保持、当前学习与资源使用评价弱监督持续分割。Domain-CL 进一步报告 E-FWT，Class-CL 进一步报告 WCD。
...
相较 Dense-Sequential，ZSDERpp 的 A-Dice 和 BWTR 分别提高 $0.0927$ 和 $0.0850$，RMA 与 E-FWT 分别低 $0.1870$ 和 $0.0350$。各方法的 MPE 均为零；结合表\ref{tab:scribblecl-methods}中的历史信息组织，可以分别观察固定模型容量下的当前监督、历史保持和最终表现。

\paragraph{三任务类别增量分割。}
ZSDERpp 与 MiB 结合后，A-Dice 为 $0.7793$，BWTR 为 $-0.0034$，RMA 为 $0.9892$，WCD 为 $0.7292$，MPE 为零。BWTR 接近零，表明旧任务从初学到最终阶段的平均相对下降较小；RMA 接近 1，表明新增任务的初学性能平均接近独立训练参照。WCD 则记录所有心脏结构进入统一输出空间后的全类别分割表现。

类别增量结果将第三章的 Class-CL 评价延伸到涂鸦监督条件。ZSDERpp 对历史特征和涂鸦样本的约束用于保留已有结构，MiB 的背景概率处理使旧类别与新增类别在阶段标签变化时保持相容。A-Dice、BWTR、RMA 和 WCD 分别反映逐任务表现、历史保持、当前学习与最终全类别输出，构成对该设置的多维评价。

\subsection{讨论}
\label{subsec:scribblecl-discussion}

全局一致性利用增强前后的预测关系约束未标注区域，空间先验利用候选非目标位置抑制不合理预测\cite{zhang2026zscribbleseg}。PCE-Sequential 与 ZS-Sequential 的比较显示，这两项约束改善了最终多域性能；ZSDERpp 进一步利用有限历史图像--涂鸦对和参考特征，在后续阶段重新优化旧任务样本并限制表征漂移。

域增量与类别增量使用相同的监督增强和特征保持机制，但变化对象不同。前者保持前景语义不变，重点考察中心适应和旧域保持；后者持续扩展心脏结构类别，需要同时处理历史类别与背景语义。对应地，MedCL 在两类设置中共用阶段矩阵、A-Dice、BWTR 和 RMA，并分别以 E-FWT 和 WCD 补充评价，使结果分析与第三章的场景定义一致。

\section{基于 MedCL 的弱监督持续分割评价}
\label{sec:scribblecl-platform-integration}
\label{sec:scribblecl-platform}
\label{sec:scribblecl-benchmark-connection}

\subsection{任务配置与阶段预测}

本章在 MedCL 中对应两类分割配置：六中心 Domain-CL 使用共享二通道输出头，三任务 Class-CL 使用共享八通道输出头。任务顺序、病例划分、标签映射和密集测试参照沿用第三章。训练监督形式、回放容量和损失组成随实验配置记录，阶段模型或预测按实际训练顺序关联到相应任务。

模型接口加载 ResUNet32 权重，对固定测试图像推理；预测接口读取与病例、切片或体数据对应的分割标签图或概率图。两类接口均按病例标识、类别编码、空间尺寸和阶段归属组织评价。训练涂鸦与回放条目用于模型更新，平台侧密集标签用于测试评分。

\subsection{阶段矩阵与多维指标汇总}

完成阶段 $t$ 后，在相应基准的固定测试任务上计算 $d_{t,j}$，形成第三章定义的阶段--任务矩阵。最终行用于 A-Dice，初学条目及最终历史任务条目用于 BWTR，初学表现与独立训练参照用于 RMA。Domain-CL 在共享标签语义下评价未来中心并计算 E-FWT；Class-CL 在最终阶段评价统一类别输出并计算 WCD。MPE 与 DRR 结合训练配置记录模型与历史信息资源。
...
\subsection{结果展示与分割可视化}

结果展示按中心或类别阶段组织同一病例的分割预测，以掩膜叠加和误差区域辅助解释 Dice 变化。中心曲线反映域增量中的适应与保持，类别视图反映新增结构及旧结构的预测变化。表\ref{tab:scribblecl-domain-results}和表\ref{tab:scribblecl-class-results}分别对应这两类场景的指标汇总，与阶段矩阵和病例可视化共同构成评价结果。

\section{本章小结}
\label{sec:scribblecl-summary}

本章在第三章 MedCL 的域增量和类别增量设置上研究弱监督持续分割，构建融合监督增强、有限图像--涂鸦回放和参考特征约束的 ZSDERpp。域增量实验沿用六中心前列腺数据，类别增量实验沿用 MMWHS 三任务设置，并结合 MiB 处理背景语义变化。

六中心实验中，ZSDERpp 将增强涂鸦顺序训练的 A-Dice 从 $0.5100$ 提高到 $0.7687$，BWTR 从 $-0.3240$ 提高到 $-0.1520$。三任务类别增量实验中，ZSDERpp + MiB 的 A-Dice、BWTR、RMA 和 WCD 分别为 $0.7793$、$-0.0034$、$0.9892$ 和 $0.7292$。两类实验共同使用 MedCL 的阶段矩阵与资源指标，将第三章的分割评价扩展到稀疏监督条件。下一章进一步转向历史原始样本不能回放的联邦分类，研究如何以跨客户端子空间摘要保持历史知识。
