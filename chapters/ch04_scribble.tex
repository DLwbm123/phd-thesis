...
第三章已经确定 MedCL 的任务组织、分割数据划分和评价规则。本章在此基础上比较持续分割方法，先考察密集监督下的跨中心适应、类别扩展与跨器官知识保持，再将三类场景的训练标签替换为固定模拟涂鸦，分析标注不足与顺序遗忘同时存在时的学习问题。
...
密集监督下的直接顺序训练仍会遗忘旧任务，而仅约束历史参数又不能补足当前图像缺少的标签。下一节因此在相同域增量、类别增量与跨器官任务数据上，用固定模拟涂鸦替代完整训练掩膜，同时研究监督增强和历史知识保持。
...
在域增量设置下，弱监督持续分割进一步考虑中心数据按阶段到达。设域序列为
...
ZS-ER & 与 ZS-Sequential 相同 & 历史图像--涂鸦经验回放 & 类别增量与跨器官比较 \\
ZS-DER & 与 ZS-Sequential 相同 & 历史图像回放与旧输出约束 & 类别增量比较 \\
ZSDerpp & 与 ZS-Sequential 相同 & 64 幅图像；水库采样；回放批大小为 8；保存涂鸦与参考特征 & 三类实验；类别增量采用 MiB 背景语义适配 \\
\bottomrule
\end{tabularx}
\end{table}
...
六中心 Domain-CL、三阶段 Class-CL 与四任务 Organ-CL 沿用第三章表\ref{tab:medcl-cross-chapter-data}的病例、任务顺序、输出头和预处理，不重新抽样。训练/验证/测试划分保持不变，只有训练监督改为固定模拟涂鸦；本节重点说明涂鸦生成及弱监督特有的训练参数。
...
\caption{三类弱监督持续分割场景的涂鸦标注与任务序列。每一行对应一种持续分割设定，箭头表示任务到达顺序；蓝色曲线表示背景涂鸦，其他彩色曲线表示当前阶段可访问的前景类别涂鸦，未被涂鸦覆盖的像素在弱监督训练中设为忽略位置。图中“器官增量”对应本文的跨器官任务增量。Domain-CL、Class-CL 和 Organ-CL 分别对应本章的六中心、三任务类别增量和四任务跨器官实验。}
...
三类实验使用第\ref{subsec:segmentation-benchmark-evaluation}节的分割评价设置。RMA 随结果记录独立训练对照的监督形式与训练预算，MPE 与 DRR 根据模型参数和历史信息记录汇总。
...
完成阶段 $t$ 后，保存模型在固定测试任务上的 Dice $d_{t,j}$。Domain-CL 测试全部中心以记录历史保持和未来域表现；Class-CL 按已见类别评分，并在最终阶段记录统一输出的 WCD；Organ-CL 在各任务对应的二通道输出上评分，记录跨器官的性能保持。评价指标沿用第\ref{subsec:segmentation-benchmark-evaluation}节，训练涂鸦和回放标签不参与测试评分。

图\ref{fig:zsderpp-domain-qualitative}、图\ref{fig:zsderpp-class-qualitative}和图\ref{fig:zsderpp-organ-qualitative}分别展示域增量、类别增量和跨器官任务增量的分割样例。同一行中的各方法使用相同图像，彩色叠加显示分割区域，预测图中的白色虚线标出真实标注轮廓。图中“密集监督”、PCE 和 ZS 分别对应表中的 Dense-Sequential、PCE-Sequential 和 ZS-Sequential；类别增量中的 ZSDerpp 采用前述 MiB 背景语义适配。图中 Dice 为所展示样例的结果，不等同于全测试集的 A-Dice 或类别增量的 WCD。

\label{subsec:scribblecl-results}
\label{subsec:scribblecl-experimental-analysis}

表\ref{tab:scribblecl-domain-results}给出六中心前列腺 MRI 序列上的 Domain-CL 结果，表\ref{tab:scribblecl-class-results}给出三任务心脏结构序列上的 Class-CL 结果，表\ref{tab:scribblecl-organ-results}给出四任务跨器官序列上的 Organ-CL 结果。三表共同报告 A-Dice、BWTR 和 RMA，分别评价最终性能、历史保持和当前任务学习。Domain-CL 进一步报告 E-FWT，Class-CL 进一步报告 WCD；Organ-CL 的任务输出语义不同，不计算这两项跨域或统一类别指标。模型扩展和历史信息使用结合 MPE、DRR 及训练配置进行分析。
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{ch04/zsderpp_domain_qualitative.pdf}
\caption[六中心域增量弱监督分割可视化]{六中心域增量弱监督分割可视化。各行依次为域 A--F 的前列腺 MRI 样例，各列为原始图像、真实标注及不同方法的预测。红色叠加表示前列腺区域，预测图中的白色虚线表示真实标注轮廓；各预测下方给出对应样例的 Dice。PCE 和 ZS 分别对应 PCE-Sequential 和 ZS-Sequential。}
\label{fig:zsderpp-domain-qualitative}
\end{figure}

相较 Dense-Sequential，ZSDerpp 的 A-Dice 和 BWTR 分别提高 $0.0927$ 和 $0.0850$，RMA 与 E-FWT 分别低 $0.1870$ 和 $0.0350$。前者采用密集监督且无历史约束，后者结合涂鸦监督、有限回放与特征保持，差异不能仅归因于标注形式。

图\ref{fig:zsderpp-domain-qualitative}进一步展示六个中心的前列腺分割结果。域 A、B 的 PCE 与 ZS 预测包含较明显的目标外区域，域 C、E 则出现目标覆盖不足。ZSDerpp 在这些样例中减少了过分割或漏分割，预测轮廓与标注更为接近，其在域 A--F 上的样例 Dice 依次为 $0.912$、$0.943$、$0.876$、$0.905$、$0.877$ 和 $0.922$。其中五个样例的 Dice 高于图中其他方法，域 D 的 $0.905$ 略低于 ZS-GPM 的 $0.910$。这些结果与表\ref{tab:scribblecl-domain-results}中较高的 A-Dice 相呼应，但不能据此认为该方法在每个中心或每个病例上均最优。

\FloatBarrier

在三任务类别增量实验中，ZSDerpp 采用 MiB 进行背景语义适配，其 A-Dice、BWTR、RMA 和 WCD 分别为 $0.7677$、$0.0031$、$0.9845$ 和 $0.7067$。其中，A-Dice、BWTR 和 WCD 均为表中最高值。BWTR 为接近零的正值，表示旧任务从初学到最终阶段的平均相对表现略有提高，但不意味着每个旧任务都没有遗忘；RMA 接近 1，表明新增任务的初学表现平均接近独立训练参照。

相较 ZS-Sequential，ZSDerpp 的 A-Dice 提高 $0.3067$，BWTR 提高 $0.6451$，WCD 提高 $0.4004$，而 RMA 低 $0.0593$。相较 Dense-Sequential，其 A-Dice 和 WCD 分别提高 $0.2833$ 和 $0.3758$，BWTR 从 $-0.6785$ 提高到 $0.0031$；Dense-Sequential 的 RMA 为 $1.1391$，仍是表中最高值。因此，ZSDerpp 的主要优势体现在旧类别保持与最终统一输出，而不是所有评价指标都最优。该比较同时改变了监督形式和历史信息使用，不能将结果差异仅归因于涂鸦或某一个损失项。

类别增量中的 ZS-ER 和 ZS-DER 分别取得 $0.3616$ 和 $0.4141$ 的 A-Dice，均低于 ZS-Sequential 的 $0.4610$；两者 BWTR 分别为 $-0.5948$ 和 $-0.5875$，虽高于 ZS-Sequential 的 $-0.6420$，仍伴随明显历史退化。有限回放本身因此不能保证最终全类别表现改善。ZSDerpp 利用历史涂鸦和参考特征保留已有结构，并处理旧类别与新增类别之间的背景语义变化；当前结果支持这一完整方案的效果，但尚不能单独量化背景适配与特征约束各自的贡献。

图\ref{fig:zsderpp-class-qualitative}按 $C_1$--$C_3$、$C_1$--$C_5$ 和 $C_1$--$C_7$ 展示三阶段心脏结构分割。随着已见类别增加，若干对比方法在图示样例中仍能预测部分新增结构，却漏分先前学习的结构，第二、三阶段尤为明显。ZSDerpp 在第二阶段仍能覆盖多个旧类别区域，在第三阶段保留了较完整的多类别预测，其三个样例的 Dice 分别为 $0.823$、$0.799$ 和 $0.678$，高于对应密集监督顺序训练的 $0.797$、$0.377$ 和 $0.227$。这种旧类别区域保留的改善，与表\ref{tab:scribblecl-class-results}中较高的 WCD 和接近零的 BWTR 所反映的趋势一致。不过，第三阶段仍存在局部结构漏分和轮廓偏差，说明类别扩展后的多结构分割仍有改进空间。

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{ch04/zsderpp_class_qualitative.pdf}
\caption[三阶段类别增量弱监督分割可视化]{三阶段类别增量弱监督分割可视化。三行分别对应已见类别集合 $C_1$--$C_3$、$C_1$--$C_5$ 和 $C_1$--$C_7$；不同颜色表示不同心脏结构，预测图中的白色虚线表示真实标注轮廓。图中“密集监督”为 Dense-Sequential；ZSDerpp 在该场景中采用 MiB 进行背景语义适配。各预测下方为对应样例的 Dice。}
\label{fig:zsderpp-class-qualitative}
\end{figure}
\FloatBarrier

四任务跨器官实验中，ZSDerpp 的 A-Dice 为 $0.8481$，BWTR 为 $-0.0252$，均为表中最高值；RMA 为 $1.0138$，接近独立训练水平，但低于 Dense-Sequential 的 $1.0757$。相较 ZS-Sequential，ZSDerpp 的 A-Dice 和 BWTR 分别提高 $0.2610$ 和 $0.3800$；相较 A-Dice 最高的无原始样本回放弱监督方法 ZS-GPM，分别提高 $0.2280$ 和 $0.3145$。这表明，在器官与模态共同变化的序列中，有限历史样本和参考特征约束能够改善最终分割表现并减轻旧任务退化。

与同样回放原始图像的 ZS-ER 相比，ZSDerpp 的 A-Dice、BWTR 和 RMA 分别提高 $0.0636$、$0.0912$ 和 $0.0065$。不过，监督增强的效果并非在所有场景中一致：Organ-CL 中 ZS-Sequential 的 A-Dice 为 $0.5871$，低于 PCE-Sequential 的 $0.6141$，BWTR 也由 $-0.2895$ 降至 $-0.4052$。这一结果与域增量中的改善形成对照，说明全局一致性和空间先验的收益受任务语义与输入分布影响，不能由单一场景直接推广。

图\ref{fig:zsderpp-organ-qualitative}依次给出左心房、前列腺、肝脏与脑肿瘤样例。左心房样例中，PCE 出现较大范围的目标外预测，ZS、ZS-EWC 和 ZS-GPM 仍有局部外溢，ZSDerpp 的预测则更接近真实区域。前列腺样例中，ZS-EWC 与 ZS-GPM 分别出现明显漏分和不连续预测，对应 Dice 为 $0.313$ 和 $0.331$，而 ZSDerpp 为 $0.793$。ZSDerpp 在四个样例上的 Dice 分别为 $0.828$、$0.793$、$0.905$ 和 $0.932$。相较 Dense-Sequential，左心房和前列腺的提升分别为 $0.084$ 和 $0.246$，大于肝脏与脑肿瘤的 $0.013$ 和 $0.005$。所展示样例的改善因此主要体现在序列中较早的任务，与表\ref{tab:scribblecl-organ-results}中较高的 BWTR 所反映的历史保持趋势一致。

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{ch04/zsderpp_organ_qualitative.pdf}
\caption[四任务跨器官弱监督分割可视化]{四任务跨器官弱监督分割可视化。各行依次为左心房、前列腺、肝脏和脑肿瘤，分别对应 Organ-CL 的任务 1--4。红色叠加显示当前任务的目标区域，预测图中的白色虚线标出真实标注轮廓。图中“密集监督”、PCE 和 ZS 分别对应 Dense-Sequential、PCE-Sequential 和 ZS-Sequential；各预测下方给出对应样例的 Dice。}
\label{fig:zsderpp-organ-qualitative}
\end{figure}
\FloatBarrier

资源方面，Class-CL 表中各方法的 MPE 均为 $0.0000$，不依赖模型参数扩展。PCE-Sequential、Dense-Sequential、ZS-Sequential 和 ZS-EWC 的 DRR 均为 $0.0000$；ZS-GPM 的 DRR 为 $0.0107$，对应历史子空间信息；ZSDerpp、ZS-ER 和 ZS-DER 的 DRR 分别为 $0.0313$、$0.0333$ 和 $0.0317$，均涉及历史原始图像回放。前者在较接近的数据使用比例下取得更高 A-Dice 与 WCD，但 DRR 不能替代回放对象的字节开销和附加计算统计。

Organ-CL 表中各方法的 MPE 均为 $5.20\times10^{-6}$，对应极小但非零的参数扩展。ZS-GPM 的 DRR 为 $0.0536$，ZS-ER 为 $0.1101$；ZSDerpp 的 DRR 为 NA，因此不参与该设置的历史数据使用比例比较。Domain-CL 的信息访问条件如表\ref{tab:scribblecl-methods}所示：顺序训练与 ZS-EWC 不回放原始图像，ZS-GPM 保留历史子空间，ZSDerpp 保留容量为 64 幅图像的图像--涂鸦回放集及参考特征。
...
密集监督与弱监督实验共享六中心前列腺、三阶段心脏结构和四任务跨器官序列的病例划分、任务顺序、预处理及测试标注，使监督条件变化前后的结果具有明确对应关系。密集监督基准用于比较正则化、回放和参数隔离的历史保持特点，弱监督实验则进一步区分当前标签不足与后续任务遗忘。PCE-Sequential 与 ZS-Sequential 的比较考察全局一致性和空间先验的整体作用，ZS-Sequential、ZS-EWC、ZS-GPM 与 ZSDerpp 的比较考察不同历史知识保持方式。

两组研究的训练条件仍需分别解释：密集监督基准中回放方法的默认容量为 32，而 ZSDerpp 使用 64 幅图像，并保存涂鸦和参考特征。密集监督主表报告均值与标准差，弱监督结果采用其固定实验配置。不同监督与资源条件共同影响实验结果，跨表差异不能单独归因于某一损失项或标注形式。
...
域增量保持前景语义不变，重点考察中心适应和旧域保持；类别增量持续扩展心脏结构，需要同时处理已见类别与阶段背景语义；跨器官任务增量改变目标器官、模态与任务输出语义。ZSDerpp 在类别增量中采用 MiB 进行背景概率适配，Organ-CL 则使用已知任务对应的二通道输出头。A-Dice、BWTR、RMA 以及适用场景下的 E-FWT 或 WCD 分别反映不同能力，不能由单一结果替代。

弱监督实验使用从密集训练掩膜生成的固定模拟涂鸦，并依赖密集验证标签选择模型。真实涂鸦的标注习惯、位置与误差可能改变监督条件；当前结果说明的是固定模拟标注下的方法表现。
...
在相同的数据划分与测试标注下，本章将三类场景的训练标签改为模拟涂鸦，提出结合监督增强、有限回放与参考特征约束的 ZSDerpp。六中心域增量中，其 A-Dice 为 $0.7687$、BWTR 为 $-0.1520$。三任务类别增量中，ZSDerpp 采用 MiB 进行背景语义适配，A-Dice、BWTR、RMA 和 WCD 分别为 $0.7677$、$0.0031$、$0.9845$ 和 $0.7067$。四任务跨器官中，ZSDerpp 的 A-Dice 为 $0.8481$、BWTR 为 $-0.0252$、RMA 为 $1.0138$。三类结果均显示最终性能与历史保持得到改善，但前向泛化、当前学习和资源指标并非同步最优。下一章转向无原始样本回放的联邦持续分类。
