\chapter{无回放条件下的联邦医学影像持续分类与工程实现研究}
...
第三章建立了对知识保持、当前任务学习、前向泛化和资源条件分别评价的思路。本章首先沿多中心协作分支展开研究：不同客户端的原始影像不能集中，历史原始样本不允许回放，而新的分类任务仍然按顺序到达。此时，知识保护既要跨越时间，也要覆盖其他客户端曾经学习的任务，单个学习器保存少量旧样本的方案不再适用。本文据此研究无回放联邦持续医学图像分类（replay-free federated continual learning，replay-free FCL）\cite{mcmahan2017fedavg,rieke2020future,yoon2021fedweit}。本章继承第三章的评价原则，但分类数据、任务身份和准确率指标单独定义，不直接复用分割的性能排名。第五章则沿任务级有限回放分支研究分割与配准，两章共享评价主线，但访问条件、模型输出和方法实现相互独立。

在这一设定下，客户端数据的非独立同分布使遗忘不再局限于单个机构。本章将全局灾难性遗忘（global catastrophic forgetting）定义为：随着新任务到达，服务器端全局模型在联邦范围内已学习任务上的性能下降。局部遗忘只描述某个客户端自身任务历史的知识损失；全局遗忘还包括其他机构曾经贡献、但当前客户端无法直接访问的历史知识。各客户端只使用当前本地数据更新模型时，类别比例、样本数量、扫描设备和人群构成等差异会使局部更新偏离联邦总体目标，聚合这些更新后，旧任务能力仍可能下降\cite{dong2022glfc}。

在无回放约束下，正则化和梯度投影可以在不回放旧图像的条件下减轻遗忘。梯度投影记忆（gradient projection memory，GPM）通过保存旧任务的重要方向，将新任务梯度投影到其正交补空间\cite{saha2021gpm}。然而，原始 GPM 面向观察完整任务序列的单一学习器。在联邦环境中，客户端独立构建的保护子空间只覆盖自身任务历史，不能直接包含仅在其他机构出现的任务方向。因而，仅在本地执行梯度投影不足以形成跨机构历史知识保护。

然而，将全部客户端方向统一加入保护空间也会产生新的约束。不同机构在标签分布、扫描仪、采集条件和任务历史上存在差异，同一历史方向对不同客户端或不同网络层的重要性并不一致。若服务器无差别地保留所有方向，可用于学习当前任务的参数更新空间会随之收缩，模型的可塑性可能下降。这里的可塑性指模型保留有效更新方向并学习新到达任务的能力。直观地说，服务器既要知道哪些历史方向值得保护，也不能把所有更新方向都锁死。因此，无回放联邦持续学习需要同时覆盖分散在不同机构的旧任务知识，并避免与目标客户端无关的过度约束。

基于上述矛盾，本章提出联邦子空间融合方法 FedSubMerge。每个客户端在当前任务结束时利用样本级梯度与服务器此前返回的历史摘要，递归构建低秩主梯度子空间（principal gradient subspace，PGS）。服务器对客户端上传的 PGS 基和奇异值进行融合，形成覆盖跨客户端历史方向的共享保护子空间。后续任务训练时，客户端将共享特征提取器的梯度投影到该子空间的正交补，以降低更新对旧任务损失的一阶干扰。在此基础上，本章提出 FedSubMerge-AD。该方法按网络层计算客户端 PGS 的投影距离，为每个目标客户端选择相关客户端，并构建客户端特定的保护子空间，以减少不必要的方向约束。

围绕上述问题，本章内容分为算法研究和工程实现两部分。算法部分建立无回放联邦持续医学图像分类的任务模型，提出由客户端 PGS 构建、服务器统一融合和正交补梯度投影组成的 FedSubMerge，并通过逐层选择相关客户端和个性化融合得到 FedSubMerge-AD，以调节跨机构知识利用与当前任务可塑性；理论与实验分别分析旧任务损失变化、递归子空间压缩、稳定性--可塑性关系、分类性能和通信开销。工程部分把客户端训练、服务器融合、阶段评价、持久化对象与后续部署接口组织为可核查的系统功能，并明确现有单机联邦实现与真实多服务器部署之间的边界。相关理论解释局部作用机制，不构成对准确率、后向迁移或形式化隐私的无条件保证。
...
在输出组织上，本章采用任务增量设定。测试时任务身份已知，并选择相应的任务特定分类头。PGS 投影作用于共享特征提取器中可训练的卷积层和全连接权重层；分类头和其他未选参数按常规方式更新。该设定不等同于测试时任务身份未知的类别增量学习，也不覆盖任务切换自动检测或开放世界识别。
...
\multirow{4}{1.4cm}{真实多中心皮肤病变} & HAM10000 & \multirow{4}{*}{6} & \multirow{4}{3.1cm}{每个来源数据集对应一个客户端} & \multirow{4}{*}{$[2,2,2]$} & \multirow{4}{*}{4} & 9041 \\
& Derm7pt & & & & & 1926 \\
& BCN20000 & & & & & 12148 \\
& DMF & & & & & 1065 \\
\bottomrule
...
\begin{table}[htbp]
\centering
\caption{标签数量偏斜与真实多中心特征偏斜设置上的 ACC 和 BWT（\%，越高越好）。红色粗体表示列最优值。$^{\ast}$ 表示回放方法。}
...
此外，不同模态上的结果还表明，本章方法的优势不依赖于单一图像类型。病理图像、腹部 CT、消化道内镜和多中心皮肤镜分别对应不同类别数、任务长度和异质性来源。所提方法在这些条件下均保持较高 ACC，说明跨客户端 PGS 融合能够用于多种医学图像分类任务；这一结论限于表\ref{tab:fedsubmerge-setup}所列任务增量设定和数据设置。
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.94\textwidth]{ch04/fedsubmerge_hyperparameter.pdf}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.94\textwidth]{ch04/fedsubmerge_accuracy_matrix.pdf}
...
在计算方面，客户端的主要附加计算集中在每个任务结束后。每个受保护层使用 $n_s=256$ 个当前任务样本构造梯度矩阵并进行截断 SVD；服务器对各层加权基进行拼接和重新压缩。FedSubMerge-AD 还需计算客户端两两投影距离并逐层选择相关客户端。该开销不会在每个本地小批量 重复执行，但随客户端数、受保护层维度和实际 PGS 秩增加。
...
\section{工程系统功能设计与实现边界}
\label{sec:fedsubmerge-engineering}

\subsection{算法模块与系统功能的对应关系}

本章前述部分给出联邦持续分类算法、理论分析和实验结果，本节从工程组织角度说明这些方法如何落到客户端训练、服务器融合和阶段评价。已有结果来自单张 NVIDIA A100 上的联邦实验实现，客户端与服务器表示实验中的逻辑角色，并不表示已经在多家医院的独立服务器上部署。图\ref{fig:fedsubmerge-engineering}将已有算法环节与后续集成所需的管理功能分开标示，避免把工程设计当作已完成的临床系统。

\begin{figure}[htbp]
\centering
\input{figures/ch04/fedsubmerge_engineering.tex}
\caption[联邦持续学习的工程流程与系统集成边界]{联邦持续学习的工程流程与系统集成边界。实线框表示本文实验中的训练、融合和评价环节；虚线框表示后续真实部署需要增加的管理与安全功能。本图为流程与接口设计，不是已部署系统的运行截图。}
\label{fig:fedsubmerge-engineering}
\end{figure}

客户端模块负责读取当前任务数据、执行本地更新，并在任务结束时构建主梯度子空间。后续任务利用服务器返回的保护方向投影梯度，历史原始图像不进入回放集合。服务器模块在常规通信轮中聚合模型，在任务结束时融合各客户端上传的基和奇异值；自适应版本进一步按网络层选择相关客户端。阶段评价模块在完成任务后记录各已见任务的准确率，形成第\ref{eq:fedsubmerge-acc-bwt}式使用的性能矩阵。三项功能共同构成现有实验的执行流程。

\subsection{信息交换、持久化对象与资源核算}

工程上需要区分两种通信节奏。模型参数在每个联邦轮次下发与上传，PGS 则只在任务结束时更新和交换。相应地，持久化的知识保护对象是各客户端最近的基、奇异值和服务器返回的保护子空间，而不是历史影像副本。任务标识、网络层名称和矩阵维度必须与被投影参数对应，否则即使矩阵运算能够执行，也可能保护错误的层或任务。真实部署中的消息校验、版本一致性和异常恢复属于系统集成需要补充的功能，不能由子空间公式自动保证。

前述资源结果为这种组织方式提供了可量化依据：在完全参与设置下，附加 PGS 通信约占 20 个常规模型通信轮次的 $3.0\%$--$3.5\%$。该比例对应实验中的传输量核算，不等于真实广域网中的端到端时延、吞吐量或故障恢复成本。不同中心的网络带宽、连接中断与计算速度可能改变实际训练进度，现有实验尚未对此提供测量。

\subsection{统一实验管理与后续部署接口}

面向全文工作的集成应优先统一实验管理，而不是强行合并不同任务的算法。共同管理项可以包括病例划分版本、任务顺序、监督类型、历史访问权限、模型版本和逐阶段评价结果。分割仍保留像素标签与 Dice 等指标，配准保留图像对、变换和 TRE 等指标，分类保留客户端标签空间与准确率。这样可以在同一管理流程中追踪“用什么数据、更新哪个模型、保留哪些历史能力”，同时保留各任务的独立评价标准。

表\ref{tab:fedsubmerge-system-boundary}区分现有证据与需要后续开发验证的部分。统一界面、真实多服务器通信、权限控制、异常重试及服务回滚尚未作为完整系统实现，因此本章不报告系统上线率、临床使用效果或并发处理性能。第\ref{sec:conclusion-future}节进一步说明从现有实验流程走向长期维护系统的研究计划。

\begin{table}[htbp]
\centering
\caption{工程集成的已有基础与后续工作边界}
...
\toprule
功能 & 本文已有基础 & 后续需要实现或验证的内容 \\
\midrule
顺序训练 & 各章固定任务流及相应方法实验 & 统一任务调度、未知任务边界与中心重访 \\
联邦更新 & 模型聚合、客户端 PGS 构建及服务器融合 & 独立服务器通信、身份认证、消息校验与断线恢复 \\
结果管理 & 按任务记录性能矩阵与资源指标 & 统一模型与数据版本索引、审计记录及可视化界面 \\
上线维护 & 已明确训练和历史信息访问条件 & 部署测试、漂移监测、人工复核与版本回滚 \\
安全性 & 不直接集中或回放历史原始影像 & 形式化隐私机制、安全评估与实际部署验证 \\
\bottomrule
\end{tabularx}
\end{table}
...
统一融合和自适应融合对应不同使用侧重。FedSubMerge 覆盖更广的历史方向，在本章七个设置中均获得高于 FedSubMerge-AD 的 BWT，更适合优先保持此前诊断任务的情况。FedSubMerge-AD 排除逐层距离较大的方向，在五个设置中取得更高 ACC，可为扫描设备、患者群体或任务历史差异较大的中心保留更多更新空间。实际应用可根据旧任务保持需求、异质性程度和通信计算预算选择相应版本。

实验覆盖病理图像、腹部 CT、消化道内镜和真实多中心皮肤镜，表明稳定性--可塑性取舍在不同模态上均可观察到。当前实验采用同步任务流、预先给定的任务划分与任务身份，并使用从头训练的 ResNet-18，相关结论适用于这些设置。
