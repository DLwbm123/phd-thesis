...
第三章 MedCL 平台以阶段--任务性能矩阵组织评价，第四章在该框架下开展密集监督分割基准评测与弱监督方法研究，第五章将 BWTR、RMA、MPE 和 DRR 用于无回放联邦分类。本章进一步研究连续空间变换的持续学习：当器官、模态和配准关系依次变化时，模型在有限历史回放条件下学习新任务，并保持已见任务的配准能力。按照 MedCL 的任务增量设置，各阶段在固定配准任务上评价，同时报告任务特定的 Dice、TRE 以及相应 BWTR、RMA 和资源指标。
...
本章先给出任务增量配准的定义和 SAMCL 方法，再通过四任务序列分析配准精度、历史保持、当前建模能力和回放资源。最后将图像对、空间变换、结构或地标参照与阶段结果对应到 MedCL，使连续变换预测与前两章的离散标签任务共享评价维度。
...
\begin{figure}[p]
\centering
\includegraphics[width=\linewidth,height=0.75\textwidth,keepaspectratio]{ch06/samcl_framework.pdf}
...
持续学习评价进一步沿用第三章的 BWTR 与 RMA，分别对 Dice 和 TRE 组织归一化指标。BWTR 按任务指标的优劣方向表示相对历史变化，负值表示性能下降，数值越高表示保持越充分；RMA 以相应任务的独立训练表现为参照，数值越高表示受限条件下的建模能力越强。Dice 与 TRE 分别汇总，使结构重叠与地标对应均能从历史保持和当前学习两个角度分析。模型参数扩展与历史信息使用则按式\eqref{eq:benchmark-mpe}和式\eqref{eq:benchmark-drr}记录 MPE 与 DRR。
...
从 MedCL 的归一化指标看，SAMCL 的 Dice-BWTR 为 $-0.0739$，高于 MER 的 $-0.0871$；Dice-RMA 为 $1.0892$，高于 MER 的 $1.0156$。在结构重叠评价中，SAMCL 因而同时具有更好的平均相对历史保持和受限建模表现。TRE 指标呈现另一种取舍：MER 的 TRE-BWTR 和 TRE-RMA 分别为 $-0.0670$ 和 $1.0177$，高于 SAMCL 的 $-0.0892$ 和 $1.0097$。这一结果与两种方法在 NLST 任务上的精度和后向变化比较相呼应，说明锐度感知更新的影响需要结合具体配准指标分析。

资源方面，MER 与 SAMCL 的 MPE 均为零，DRR 分别为 $0.2020$ 和 $0.1806$，SAMCL 低 $0.0214$。结合任务性能、BWTR 和 RMA，可以在固定模型参数规模下同时考察历史信息使用与新旧任务表现，从而将第三章的多维评价落实到配准任务。
...
\begin{figure}[p]
\centering
\includegraphics[width=\textwidth,height=0.81\textheight,keepaspectratio]{ch06/samcl_qualitative_oasis_large.pdf}
\caption[不同训练策略的代表性配准结果]{不同训练策略在 OASIS 代表性图像对上的配准结果。两组方法分别放大展示，每组左侧重复给出同一固定图像与移动图像作为参照；各方法的上排为差异图，下排为配准结果及对应 Dice。腹部 CT--CT 结果见续图。}
...
\centering
\includegraphics[width=\textwidth,height=0.81\textheight,keepaspectratio]{ch06/samcl_qualitative_ctct_large.pdf}
\caption{不同训练策略的代表性配准结果（续）：腹部 CT--CT。两组方法分别放大展示，每组左侧为同一固定图像与移动图像；各方法的上排为差异图，下排为配准结果及对应 Dice。CT 图像显示时将强度裁剪到 $[-200,300]$。}
\end{figure}
}{}
...
\begin{figure}[p]
\centering
\includegraphics[width=\linewidth,height=0.75\textwidth,keepaspectratio]{ch06/plot_memory_nnn.pdf}
\caption[SAMCL 的回放容量与泛化实验]{SAMCL 的回放容量与泛化实验。（a）不同回放容量下的配准结果。OASIS、CTCT 和 MRCT 报告 Dice，NLST 报告 TRE；CTCT 表示腹部 CT--CT 配准，MRCT 表示腹部 MR--CT 配准。泛化实验见续图。}
...
\captionsetup{list=off}
\centering
\includegraphics[width=\linewidth,height=0.75\textwidth,keepaspectratio]{ch06/plot_sam_new.pdf}
\caption{SAMCL 的回放容量与泛化实验（续）。（b）采用或不采用锐度感知最小化时的域内与跨任务泛化结果。CTCT 表示腹部 CT--CT 配准，MRCT 表示腹部 MR--CT 配准。}
\end{figure}
\end{landscape}
...
本章在原始 Dice、TRE 和任务后向变化之外，分别报告 Dice-BWTR、TRE-BWTR、Dice-RMA 和 TRE-RMA，并结合 MPE、DRR 记录资源使用。本章因此与第四章的持续分割研究和第五章的联邦分类共享历史保持、当前学习和资源评价维度，同时保留配准的结构与地标指标。
...
本章将第三章的阶段矩阵、BWTR、RMA、MPE 和 DRR 用于持续配准。SAMCL 的 Dice-BWTR 和 Dice-RMA 分别为 $-0.0739$ 和 $1.0892$，高于 MER 的 $-0.0871$ 和 $1.0156$；TRE 指标则体现 MER 的相对优势。SAMCL 的 MPE 为零，DRR 为 $0.1806$。至此，第四至第六章分别在持续分割基准与弱监督方法、无回放联邦分类和有限回放配准中使用 MedCL 的多维评价，形成从离散标签预测到连续空间变换的研究联系。
