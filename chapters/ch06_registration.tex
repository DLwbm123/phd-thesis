...
前两章研究分割与分类中的离散标签预测，本章转向配准中的连续空间变换。器官、模态和配准关系依次变化时，模型需要学习新的对齐方式，同时保留已见任务的配准能力。本章允许保存少量历史图像对，并沿用第三章 MedCL 的任务增量设定与阶段矩阵，分别记录 Dice、TRE、BWTR、RMA 和资源指标。
...
为协调当前任务适应与历史任务保持，本章研究锐度感知元持续医学图像配准方法 SAMCL\cite{wang2024samcl}。SAMCL 在有限经验回放基础上，利用一阶元持续学习促进当前样本与历史样本更新方向的一致性，并把锐度感知优化嵌入元更新，使模型同时关注当前参数点及其邻域中的配准损失。实验按照 OASIS 脑部 MR--MR、腹部 CT--CT、NLST 肺部 CT--CT 和腹部 MR--CT 的固定顺序，比较直接顺序训练、参数正则、梯度投影、元经验回放以及完整数据访问参照，并分别报告 Dice、TRE、后向变化、BWTR、RMA 和预设未见任务方向上的直接表现，并分析回放容量对配准结果的影响。

本章先给出任务增量配准的定义和 SAMCL 方法，再通过四任务序列的平均性能、结果分布与代表性图像对考察配准精度、历史保持和当前学习，并分析回放容量与泛化表现，最后说明图像对、空间变换和评价参照在 MedCL 中的记录方式，讨论方法的适用条件。
...
\begin{figure}[htbp]
\centering
\thesisgraphic{ch06/samcl_framework.pdf}
...
表\ref{tab:samcl-datasets}给出四项任务的数据组织。OASIS、腹部 CT--CT 和腹部 MR--CT 分别使用 35、13 和 9 个对应解剖标签评价配准后的结构重叠；NLST 使用对应地标评价目标配准误差。表中的训练、验证和测试数量与本文前期实验设置保持一致\cite{wang2024samcl}，数量按原论文的三维影像数报告。
...
对于 OASIS、腹部 CT--CT 和腹部 MR--CT，实验在相应解剖标签上计算平均 Dice；对于 NLST，在对应地标上计算以毫米为单位的目标配准误差（target registration error, TRE）。沿用第三章式\eqref{eq:medcl-evaluation-matrix}，令 $R_{i,j}$ 表示完成任务 $i$ 后在任务 $j$ 测试集上的评价。任务 $j$ 的最终性能及其相对于初学阶段的后向变化定义为
\begin{equation}
...
\end{equation}
对 Dice，BWT 为负表示重叠精度下降，因此 Dice-BWT 越高越好；对 TRE，BWT 为正表示误差增大，因此 TRE-BWT 越低越好。最后一个任务之后没有进一步训练，其 BWT 按定义为零。原始任务性能保留各自的方向与单位。

持续学习评价进一步沿用第三章的 BWTR 与 RMA，分别对 Dice 和 TRE 组织归一化指标。BWTR 按第\ref{sec:benchmark-metrics}节的指标方向计算，负值表示性能下降，数值越高表示保持越充分；RMA 以相应任务的独立训练表现为对照。Dice 使用初学性能与独立训练性能之比，TRE 使用独立训练误差与初学误差之比，均以较大值表示较强的受限建模能力。两类指标分别汇总，不混合结构重叠与地标误差。

四项任务仍按原序列编号，归一化汇总仅对适用任务取平均。Dice-BWTR 使用 OASIS 和腹部 CT--CT 两项历史任务，Dice-RMA 使用腹部 CT--CT 和腹部 MR--CT 两项后续任务；TRE-BWTR 与 TRE-RMA 均只使用 NLST。具体地，初学性能由 $R_{j,j}=\mathrm{AVG}_j-\mathrm{BWT}_j$ 得到，再分别计算相对后向变化和相对于独立训练的性能比值。表\ref{tab:samcl-main-results}中的归一化结果根据三位小数的任务指标计算，并保留四位小数，因此为近似汇总值。例如，SAMCL 在 OASIS 和腹部 CT--CT 上的初学 Dice 分别为 $0.734$ 和 $0.391$，在 NLST 上的初学 TRE 为 $3.549$ mm；Dice-BWTR 对前两项任务的相对 Dice 变化取平均，TRE-BWTR 则为 $-0.306/3.549\approx-0.0862$。历史访问条件由表\ref{tab:samcl-baselines}说明，回放容量与训练预算在实现配置中单独记录。
...
表\ref{tab:samcl-main-results}给出四任务顺序训练结束后的最终性能、后向变化以及 BWTR 与 RMA。直接顺序训练在三个历史任务上均出现性能下降：OASIS 和腹部 CT--CT 的 Dice-BWT 分别为 $-0.395$ 和 $-0.129$，NLST 的 TRE-BWT 为 $3.508$ mm。SAMCL 将这三项后向变化改善为 $-0.037$、$-0.039$ 和 $0.306$ mm；相应的最终 OASIS Dice 从 $0.348$ 提高到 $0.697$，腹部 CT--CT Dice 从 $0.270$ 提高到 $0.352$，NLST TRE 从 $6.922$ mm 降至 $3.855$ mm。该结果表明，有限回放、元持续更新与锐度感知训练的组合能够缓解直接顺序训练造成的历史性能下降，但并未完全消除遗忘。

\input{tables/SAMCL}

与 SI 和 GPM 相比，SAMCL 在三项 Dice 任务上均取得更高的最终重叠精度，在 NLST 上取得更低的 TRE，三个历史任务的后向退化也更小。与 EWC 相比，SAMCL 在 OASIS 上的最终 Dice 高 $0.038$，Dice-BWT 高 $0.046$；但腹部 CT--CT 上 EWC 的最终 Dice 为 $0.364$、Dice-BWT 为 $-0.011$，均优于 SAMCL 的 $0.352$ 和 $-0.039$。NLST 上，EWC 的最终 TRE 为 $3.836$ mm，略低于 SAMCL 的 $3.855$ mm，其 TRE-BWT 却为 $0.386$ mm，高于 SAMCL 的 $0.306$ mm。这说明较低的最终误差不必然对应较小的后续误差增长，配准精度与历史保持需要分别评价。

MER 与 SAMCL 均使用有限历史图像对回放，二者的优势随配准任务变化。OASIS 上，MER 的最终 Dice 为 $0.698$，仅比 SAMCL 高 $0.001$，Dice-BWT 也仅高 $0.001$。腹部 CT--CT 和腹部 MR--CT 上，SAMCL 的最终 Dice 分别比 MER 高 $0.021$ 和 $0.031$。其中，CT--CT 的初学 Dice 可由最终性能与后向变化反推：SAMCL 为 $0.391$，MER 为 $0.371$；二者随后分别下降 $0.039$ 和 $0.040$。因此，最终 $0.021$ 的差值主要对应初学表现的差异，不能只解释为遗忘减少。MR--CT 为最后一个任务，二者的后向变化均为零，其差异同样主要反映当前任务适应。

NLST 上，MER 的 TRE 为 $3.611$ mm、TRE-BWT 为 $0.077$ mm，分别低于 SAMCL $0.244$ mm 和 $0.229$ mm。综合 EWC、SI、GPM、MER 与 SAMCL 五种持续学习方法，OASIS 最终 Dice 和 NLST 最终 TRE 的最优值由 MER 取得，腹部 CT--CT 最终 Dice 的最优值由 EWC 取得，腹部 MR--CT 最终 Dice 的最优值由 SAMCL 取得。因此，不能将 SAMCL 在腹部 Dice 任务上的改进概括为所有配准任务均占优。

从归一化指标看，SAMCL 的 Dice-BWTR 约为 $-0.0751$，高于 MER 的 $-0.0784$；Dice-RMA 约为 $1.0954$，高于 MER 的 $1.0374$。在五种持续学习方法中，SAMCL 的 Dice-RMA 最高，但 Dice-BWTR 仍低于 EWC 的 $-0.0706$。Dice-RMA 汇总腹部 CT--CT 与腹部 MR--CT 的初学性能，Dice-BWTR 汇总 OASIS 与腹部 CT--CT 的相对历史变化，不能当作同一组任务的单一综合分数。直接顺序训练的 Dice-RMA 为 $1.1119$，高于 SAMCL，但 Dice-BWTR 仅为 $-0.4275$。这进一步表明，较强的新任务初学表现可能伴随严重的历史退化。

TRE 的归一化指标均来自 NLST。MER 的 TRE-BWTR 和 TRE-RMA 分别约为 $-0.0218$ 和 $0.9222$，均高于 SAMCL 的 $-0.0862$ 和 $0.9183$；SAMCL 的 TRE-BWTR 则高于 EWC 的 $-0.1119$。SAMCL 与 MER 的 NLST 初学 TRE 分别为 $3.549$ mm 和 $3.534$ mm，随后误差分别增加 $0.306$ mm 和 $0.077$ mm，最终形成 $3.855$ mm 与 $3.611$ mm 的差异。两者的初学误差接近，后续误差增长的差异更大。

直接顺序训练的 TRE-RMA 为 $0.9546$，高于五种持续学习方法，但其 TRE-BWTR 为 $-1.0275$，表示最终误差比初学时增加约 $102.75\%$。所有顺序学习方法的 TRE-RMA 均低于 $1$，说明到达 NLST 时的初学误差仍高于独立训练的 $3.259$ mm。TRE、TRE-BWT、TRE-BWTR 和 TRE-RMA 因此分别描述最终精度、绝对误差变化、相对误差变化和初学能力，不能相互替代。

方法间的比较还需保留历史访问条件。EWC、SI 和 GPM 不回放历史原始图像，MER 与 SAMCL 则保存容量受限的历史图像对。即使采用相同骨干，不同方法的历史信息形式与更新过程也不同，不能仅凭任务性能推断存储或计算成本。回放容量对配准表现的影响在第\ref{subsubsec:samcl-ablation-generalization}节单独分析。

独立训练使用每项任务的专用模型，集中式多任务训练同时访问全部任务数据，二者不与持续学习方法视为相同训练条件。SAMCL 在 OASIS 上低于这两项参照，在腹部 CT--CT 上接近集中式多任务训练，在腹部 MR--CT 上则高于两项参照。因此，集中式多任务训练是完整数据访问参照，而非表中每项指标的数值上界。直接顺序训练在最后任务上取得 $0.568$ 的 Dice，略高于 SAMCL 的 $0.563$；由于此后没有进一步训练，该差异只反映当前任务适应。在三个历史任务上，SAMCL 均缩小了直接顺序训练与集中式多任务参照之间的差距，但仍存在不同程度的性能损失。

图\ref{fig:samcl-performance-distributions}进一步展示直接顺序训练与五种持续学习方法在四项任务上的测试性能分布，以补充表\ref{tab:samcl-main-results}中的平均结果。小提琴的宽度反映相应性能区间的结果密度，内部箱线概括分布的中心位置与离散程度；独立训练和集中式多任务训练的参考结果见表\ref{tab:samcl-main-results}。

\begin{figure}[htbp]
\centering
\thesisgraphic{ch06/samcl_performance_distributions.pdf}
\caption[四项配准任务的性能分布]{直接顺序训练与五种持续学习方法在四项配准任务上的性能分布。（a）OASIS；（b）腹部 CT--CT；（c）NLST；（d）腹部 MR--CT。Seq. 表示直接顺序训练。OASIS、腹部 CT--CT 和腹部 MR--CT 使用 Dice，数值越高越好；NLST 使用 TRE，单位为 mm，数值越低越好。}
\label{fig:samcl-performance-distributions}
\end{figure}

在 OASIS 上，SAMCL 与 MER 的分布集中于较高 Dice 区域，二者有较大重叠；直接顺序训练、SI 和 GPM 的主体分布则位于更低的重叠精度区间。这说明表\ref{tab:samcl-main-results}中早期脑部任务的平均性能差异也体现在整体分布位置上，而不仅体现为个别较优结果。腹部 CT--CT 上，各方法的分布重叠更多，SAMCL 的主要密集区域高于直接顺序训练与 GPM，但与 EWC、MER 仍有明显重叠。结合主表中 EWC、SAMCL 和 MER 分别为 $0.364$、$0.352$ 和 $0.331$ 的平均 Dice，可见分布虽有重叠，平均精度仍存在差异，SAMCL 并未取得最高平均精度。

在 NLST 上，EWC、MER 与 SAMCL 的 TRE 主体分布位于较低误差区间，与直接顺序训练、SI 和 GPM 形成明显差异。然而，较低的分布中心仍可能伴随高误差尾部，表明平均误差降低并不意味着所有测试结果都同样准确。MER 与 SAMCL 的低误差区域有较大重叠，主表中 MER 的平均 TRE 仍低于 SAMCL。腹部 MR--CT 上，不同策略的 Dice 分布重叠较多，SAMCL 的分布也较宽，反映出跨模态配准表现的差异。由于 MR--CT 是最后到达的任务，该分布主要反映当前任务学习，而非后续训练造成的遗忘。
...
\begin{figure}[htbp]
\centering
\thesisgraphic{ch06/samcl_qualitative.pdf}
\caption[不同训练策略的代表性配准结果]{不同训练策略在 OASIS 与腹部 CT--CT 代表性图像对上的配准结果。每个任务的上排为固定图像与配准后移动图像的差异图，下排为配准结果及对应 Dice；左侧为固定图像与原始移动图像。CT 图像显示时将强度裁剪到 $[-200,300]$。}
\label{fig:samcl-qualitative}
\end{figure}
}{}

图\ref{fig:samcl-qualitative-nlst}给出 NLST 肺部 CT--CT 的代表性图像对，补充观察以地标误差评价的配准任务。直接顺序训练、SI 和 GPM 的差异图在肺底及膈肌附近仍存在较明显的红蓝残差，SAMCL 与 MER 在相应区域的残差相对减弱，配准后图像的肺部轮廓也更接近固定图像。该图像对上，直接顺序训练、SI 和 GPM 的 TRE 分别为 $7.915$、$8.908$ 和 $9.421$ mm，SAMCL 为 $3.242$ mm，相较直接顺序训练降低 $4.673$ mm。SAMCL 的误差仍高于独立训练的 $3.018$ mm 与集中式多任务训练的 $2.611$ mm，表明有限历史回放与完整数据访问参照之间仍存在差距。

\begin{figure}[htbp]
\centering
\thesisgraphic{ch06/samcl_qualitative_nlst.pdf}
\caption[NLST 代表性图像对的配准结果]{不同训练策略在 NLST 代表性图像对上的配准结果。上下两组分别展示四种训练策略，每组左列均为固定图像与原始移动图像；其余列的上排为固定图像与配准后移动图像的差异图，下排为配准后的移动图像，并标注该图像对的 TRE，单位为 mm。}
\label{fig:samcl-qualitative-nlst}
\end{figure}

在该 NLST 图像对上，SAMCL 的 TRE 比 MER 的 $3.401$ mm 低 $0.159$ mm；而在表\ref{tab:samcl-main-results}的测试集平均结果中，MER 的 $3.611$ mm 低于 SAMCL 的 $3.855$ mm。两种比较的次序不同，说明方法间的相对表现随具体图像对变化。代表性图像用于定位可见的对齐差异，性能分布用于观察结果的集中位置与离散程度，测试集均值及后向变化则分别概括最终精度和历史保持，三者从不同层面补充配准评价。
...
图\ref{fig:samcl-ablation-generalization}(a)比较回放容量为 $50$、$100$ 和 $200$ 时的配准结果。OASIS 和腹部 CT--CT 的 Dice 中位数随容量增加而提高，NLST 的 TRE 中位数下降；腹部 MR--CT 的分布重叠较多，未呈现同样明确的单调变化。回放容量对不同任务的影响并不完全一致，方法比较时需同时报告保存的图像对数量。
...
\begin{figure}[htbp]
\centering
\begin{minipage}{\thesisfigurewidth}
\centering
\includegraphics[width=\linewidth,height=0.30\textheight,keepaspectratio]{ch06/plot_memory_nnn.pdf}\par
{\small （a）回放容量}\par\vspace{6pt}
\includegraphics[width=\linewidth,height=0.30\textheight,keepaspectratio]{ch06/plot_sam_new.pdf}\par
{\small （b）域内与跨任务泛化}
\end{minipage}
\caption[SAMCL 的回放容量与泛化实验]{SAMCL 的回放容量与泛化实验。（a）不同回放容量下的配准结果；（b）采用或不采用锐度感知最小化时的泛化表现。OASIS、CTCT 和 MRCT 报告 Dice，NLST 报告 TRE；CTCT 与 MRCT 分别表示腹部 CT--CT 和腹部 MR--CT 配准。}
\label{fig:samcl-ablation-generalization}
\end{figure}

回放容量实验直接考察历史图像对数量对配准性能的影响，域内及跨任务测试则补充观察给定配准方向上的泛化表现。容量变化不能直接换算为存储字节量或计算时间，二者还取决于图像表示和具体训练实现。
...
本章沿用第三章的任务与阶段组织，将 OASIS、腹部 CT--CT、NLST 和腹部 MR--CT 分别登记为固定任务，并将阶段模型或配准预测关联到实际训练顺序。模型接口加载配准网络权重，预测接口读取与测试图像对一一对应的空间变换，二者均使用固定测试图像对与结构或地标参照进行评价。

每个样本记录固定图像、移动图像、体素间距、空间方向、预处理和变换约定。本文网络采用固定域到移动域的采样映射，以 $I_M\circ\phi$ 得到固定网格上的变形图像；位移场同时记录分量顺序、单位及其与恒等映射的关系。这些任务信息使阶段之间使用相同的重采样与评分过程。

\begin{table}[htbp]
\centering
\caption{持续配准与 MedCL 任务配置及评价指标的对应关系}
...
评价对象 & 数据与配置 & 对应输出 \\
\midrule
任务与阶段 & 器官、模态、图像对及到达顺序 & 阶段模型、任务标识与性能矩阵 \\
空间变换 & 固定--移动方向、坐标、体素间距与单位 & 变形图像、结构标签与地标位置 \\
配准精度 & 对应结构标签与物理坐标地标 & 任务 Dice 与毫米级 TRE \\
保持与学习 & 最终、初学及独立训练参照 & 分指标的 BWT、BWTR 与 RMA \\
资源使用 & 历史图像对、回放配置与训练预算 & 回放对象、容量与训练迭代数 \\
\bottomrule
\end{tabularx}
\end{table}

\subsection{任务性能与持续学习指标}

各阶段模型在固定测试任务上的结果组成性能矩阵。OASIS、腹部 CT--CT 和腹部 MR--CT 以结构 Dice 评价解剖重叠，NLST 以毫米级 TRE 评价地标对应。最终性能记录于矩阵末行，历史变化由相应初学条目与末行条目计算，当前建模能力以独立训练模型为参照。

本章在原始 Dice、TRE 和任务后向变化之外，分别报告 Dice-BWTR、TRE-BWTR、Dice-RMA 与 TRE-RMA，回放对象、容量和训练预算另行说明。该组织方式既区分当前学习与历史保持，也保留结构重叠和地标误差各自的含义，不将两类任务指标混合平均。

\subsection{配准可视化与阶段比较}

可视化按测试图像对并列展示固定图像、原始移动图像和变形图像，通过切片联动、叠加或棋盘格观察对齐关系，并结合有效空间变换显示形变网格。各阶段使用相同图像对、参考标签和地标，便于观察旧任务对齐能力的变化。任务结果同时标明 Dice、TRE 的方向和单位，归一化指标则用于概括历史保持与当前建模表现。

\subsection{方法适用条件与局限}
\label{sec:registration-discussion}

这些评价也有助于说明方法的适用条件。同一个配准模型虽然始终输出空间变换，但不同任务的图像相似性、结构参照和形变分布并不相同，不能仅凭平均结果判断每项任务的适应与保持效果。

在固定四任务顺序下，SAMCL 减轻了直接顺序训练在前三个任务上的性能下降。与 MER 比较时，两者在 OASIS 上接近，SAMCL 在腹部 CT--CT 和腹部 MR--CT 上的 Dice 较高，MER 在 NLST 上的 TRE 及其后向变化较低。 图\ref{fig:samcl-performance-distributions}中的分布重叠和图\ref{fig:samcl-qualitative-nlst}中的个例比较进一步表明，平均性能上的相对优势不等同于在每个图像对上均占优。因此，任务均值、性能分布和局部对齐结果需要结合分析。

泛化比较覆盖两项域内测试和三项跨任务测试。任务序列中的器官、模态、配准关系和损失配置共同影响模型更新；按照 MedCL 分别记录任务表现、历史保持和当前建模能力，有助于分析这些变化下的学习取舍。

SAMCL 保存有限历史原始图像对，其存储与计算需求受回放容量、体数据分辨率和更新过程影响。现有容量实验说明了不同回放规模下的性能变化，但不能代替存储字节量、运行时间或峰值显存的直接测量。因此，任务性能优势不等于在所有资源条件下均具有更高效率。

\section{本章小结}
\label{sec:registration-summary}

本章提出 SAMCL，将有限图像对回放、一阶元持续学习与锐度感知优化结合，用于跨器官和模态的持续配准。四任务实验表明，SAMCL 改善了直接顺序训练的历史保持，与 MER 的相对表现则随任务和指标变化。性能分布与代表性配准结果补充了均值之外的差异；容量消融及泛化测试考察了回放规模与锐度感知更新的作用。

本章沿用第三章的阶段矩阵、BWTR 与 RMA，并区分 Dice 和 TRE 的方向与单位。SAMCL 的 Dice-BWTR 和 Dice-RMA 分别约为 $-0.0751$ 和 $1.0954$，均高于 MER，但 EWC 的 Dice-BWTR 更高，MER 在 NLST 的最终 TRE、后向变化及归一化指标上更优。SAMCL 在两项腹部任务上的较高最终 Dice，不代表其在所有任务上遗忘更少。