...
第三章 MedCL 平台以阶段--任务性能矩阵组织评价，第四章在弱监督分割中沿用其数据、设定和指标，第五章将 BWTR、RMA、MPE 和 DRR 用于无回放联邦分类。本章进一步研究连续空间变换的持续学习：当器官、模态和配准关系依次变化时，模型在有限历史回放条件下学习新任务，并保持已见任务的配准能力。按照 MedCL 的任务增量设置，各阶段在固定配准任务上评价，同时报告任务特定的 Dice、TRE 以及相应 BWTR、RMA 和资源指标。
...
本章在原始 Dice、TRE 和任务后向变化之外，分别报告 Dice-BWTR、TRE-BWTR、Dice-RMA 和 TRE-RMA，并结合 MPE、DRR 记录资源使用。表\ref{tab:samcl-main-results}因此与第四章的弱监督分割表和第五章的联邦分类表共享历史保持、当前学习和资源评价维度，同时保留配准的结构与地标指标。
...
本章将第三章的阶段矩阵、BWTR、RMA、MPE 和 DRR 用于持续配准。SAMCL 的 Dice-BWTR 和 Dice-RMA 分别为 $-0.0739$ 和 $1.0892$，高于 MER 的 $-0.0871$ 和 $1.0156$；TRE 指标则体现 MER 的相对优势。SAMCL 的 MPE 为零，DRR 为 $0.1806$。至此，第四至第六章分别在弱监督分割、无回放联邦分类和有限回放配准中使用 MedCL 的多维评价，形成从离散标签预测到连续空间变换的研究联系。
