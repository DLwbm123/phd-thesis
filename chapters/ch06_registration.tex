...
本章先给出任务增量配准的定义和 SAMCL 方法，再通过四任务序列考察配准精度、历史保持、当前学习与回放容量，最后说明图像对、空间变换和评价参照在 MedCL 中的记录方式，并讨论方法的适用条件。
...
持续学习评价进一步沿用第三章的 BWTR 与 RMA，分别对 Dice 和 TRE 组织归一化指标。BWTR 按第\ref{sec:benchmark-metrics}节的指标方向计算，负值表示性能下降，数值越高表示保持越充分；RMA 以相应任务的独立训练表现为对照。Dice 使用初学性能与独立训练性能之比，TRE 使用独立训练误差与初学误差之比，均以较大值表示较强的受限建模能力。Dice 与 TRE 分别汇总，使结构重叠与地标对应均能从历史保持和当前学习两个角度分析。模型参数扩展与历史信息使用则按式\eqref{eq:benchmark-mpe}和式\eqref{eq:benchmark-drr}记录 MPE 与 DRR。
...
资源方面，MER 与 SAMCL 的 MPE 均为零，DRR 分别为 $0.2020$ 和 $0.1806$，SAMCL 低 $0.0214$。两种方法都未增加模型参数，历史信息使用比例却不同，因此比较配准结果时还需考虑实际回放条件。
...
本章将第三章的阶段矩阵、BWTR、RMA、MPE 和 DRR 用于持续配准。SAMCL 的 Dice-BWTR 和 Dice-RMA 分别为 $-0.0739$ 和 $1.0892$，高于 MER 的 $-0.0871$ 和 $1.0156$；TRE 指标则体现 MER 的相对优势。SAMCL 的 MPE 为零，DRR 为 $0.1806$。配准实验与前两章一样，保留原始任务指标，再用阶段变化和资源记录分析持续更新的效果。
