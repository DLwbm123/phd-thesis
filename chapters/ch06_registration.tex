...
为协调当前任务适应与历史任务保持，本章研究锐度感知元持续医学图像配准方法 SAMCL\cite{wang2024samcl}。SAMCL 在有限经验回放基础上，利用一阶元持续学习促进当前样本与历史样本更新方向的一致性，并把锐度感知优化嵌入元更新，使模型同时关注当前参数点及其邻域中的配准损失。实验按照 OASIS 脑部 MR--MR、腹部 CT--CT、NLST 肺部 CT--CT 和腹部 MR--CT 的固定顺序，比较直接顺序训练、参数正则、梯度投影、元经验回放以及完整数据访问参照，并分别报告 Dice、TRE、后向变化、BWTR、RMA、MPE、DRR 和预设未见任务方向上的直接表现。
...
表\ref{tab:samcl-main-results}给出四任务顺序训练结束后的最终性能、后向变化及 MedCL 的 BWTR、RMA、MPE 和 DRR。直接顺序训练在前三个任务上出现明显知识损失：OASIS 和腹部 CT--CT 的 Dice-BWT 分别为 $-0.395$ 和 $-0.129$，NLST 的 TRE-BWT 为 $3.508$ mm。SAMCL 将三项后向变化改善为 $-0.037$、$-0.039$ 和 $0.306$ mm，说明有限回放、元持续更新与锐度感知训练的组合能够明显缓解直接顺序训练的遗忘。

\input{tables/SAMCL}

从最终配准精度看，SAMCL 在 OASIS、腹部 CT--CT 和腹部 MR--CT 上分别取得 $0.697$、$0.352$ 和 $0.563$ 的 Dice，在 NLST 上取得 $3.855$ mm 的 TRE。相较 SI 和 GPM，SAMCL 在三项 Dice 任务上均具有更高最终重叠精度，在 NLST 上也具有更低 TRE，并减小前三个任务的后向退化。与 EWC 相比，SAMCL 在 OASIS 上的最终 Dice 高 $0.038$，Dice-BWT 高 $0.046$；腹部 CT--CT 上 EWC 的最终 Dice 为 $0.364$，高于 SAMCL 的 $0.352$，Dice-BWT 为 $-0.011$，也优于 SAMCL 的 $-0.039$。NLST 上 EWC 的最终 TRE 为 $3.836$ mm，略低于 SAMCL 的 $3.855$ mm，但其 TRE-BWT 为 $0.386$ mm，高于 SAMCL 的 $0.306$ mm。最终误差与后向误差变化因此不能相互替代。该结果说明参数正则化在某一任务上仍可能有效，但其表现会随器官和损失形式变化。

MER 与 SAMCL 均使用有限回放。MER 的 OASIS 最终 Dice 为 $0.698$，比 SAMCL 高 $0.001$；SAMCL 的腹部 CT--CT 和腹部 MR--CT Dice 分别高 $0.021$ 和 $0.031$。两者在 OASIS 和 CT--CT 上的 Dice-BWT 差值均为 $0.001$，方向分别有利于 MER 和 SAMCL。NLST 上 MER 的 TRE 为 $3.611$ mm、TRE-BWT 为 $0.077$ mm，分别低于 SAMCL $0.244$ mm 和 $0.229$ mm。在 EWC、SI、GPM、MER 与 SAMCL 五种持续学习方法中，OASIS 最终 Dice 和 NLST 最终 TRE 的最优值由 MER 取得，CT--CT 最终 Dice 的最优值由 EWC 取得，MR--CT 最终 Dice 的最优值由 SAMCL 取得，不同持续学习方法的优势随配准任务变化。

按表中归一化汇总结果，SAMCL 的 Dice-BWTR 约为 $-0.0751$，高于 MER 的 $-0.0784$，差值约为 $0.0033$；Dice-RMA 约为 $1.0954$，高于 MER 的 $1.0374$，差值约为 $0.0580$。在 EWC、SI、GPM、MER 与 SAMCL 中，SAMCL 的 Dice-RMA 最高，但 Dice-BWTR 低于 EWC 的 $-0.0706$。EWC 在腹部 CT--CT 上的相对性能下降较小，使其两项历史 Dice 任务的平均保持率高于 SAMCL。直接顺序训练的 Dice-RMA 为 $1.1119$，同时 Dice-BWTR 仅为 $-0.4275$，表明较强的初学适应并不等于较好的历史保持。

TRE 的归一化结果同样区分当前学习与后续保持。MER 的 TRE-BWTR 和 TRE-RMA 分别约为 $-0.0218$ 和 $0.9222$，均高于 SAMCL 的 $-0.0862$ 和 $0.9183$；SAMCL 的 TRE-BWTR 则高于 EWC 的 $-0.1119$。直接顺序训练的 TRE-RMA 为 $0.9546$，高于各持续学习方法，但其 TRE-BWTR 为 $-1.0275$，表示最终误差比初学时增加约 $102.75\%$。各方法的 TRE-RMA 均低于 $1$，说明顺序学习到 NLST 时的初学误差仍高于独立训练的 $3.259$ mm。SAMCL 的 NLST 初学 TRE 为 $3.549$ mm，略高于 MER 的 $3.534$ mm，后续误差又分别增加 $0.306$ mm 和 $0.077$ mm，最终形成 $3.855$ mm 与 $3.611$ mm 的差异。因此，最终 TRE、TRE-BWT、TRE-BWTR 和 TRE-RMA 分别描述最终精度、绝对误差变化、相对误差变化和初学能力。

资源方面，直接顺序训练及 EWC、SI、GPM、MER、SAMCL 的 MPE 均为 $0.0000$，没有模型参数扩展。直接顺序训练、EWC 和 SI 的 DRR 均为 $0.0000$；GPM 的 DRR 为 $0.4987$，对应历史子空间信息，而非原始图像对回放。MER 与 SAMCL 的 DRR 分别为 $0.2020$ 和 $0.1806$，SAMCL 低 $0.0214$，即低 $2.14$ 个百分点。DRR 衡量历史信息构建与复用所涉及的数据比例；存储字节量和优化计算量还与回放对象及更新策略有关。
...
\centering
\includegraphics[width=\linewidth,height=0.27\textheight,keepaspectratio]{ch06/plot_memory_nnn.pdf}\par
{\small （a）回放容量}\par\vspace{6pt}
\includegraphics[width=\linewidth,height=0.27\textheight,keepaspectratio]{ch06/plot_sam_new.pdf}\par
{\small （b）域内与跨任务泛化}
\end{minipage}
...
回放容量分析与 MedCL 的 DRR 共同描述历史信息使用，域内及跨任务测试则补充观察模型在给定配准方向上的泛化表现。
...
资源使用 & 模型参数与历史图像对使用记录 & MPE、DRR 和回放容量 \\
\bottomrule
\end{tabularx}
\end{table}
...
本章在原始 Dice、TRE 和任务后向变化之外，分别报告 Dice-BWTR、TRE-BWTR、Dice-RMA 与 TRE-RMA，并以 MPE、DRR 记录资源使用。这样既能与前两章一样区分学习、保持和资源条件，也不会混淆结构重叠与地标误差的含义。
...
SAMCL 保存有限历史原始图像对，其存储与计算需求随回放容量变化。MPE 和 DRR 分别描述模型扩展及历史信息使用，Dice 和 TRE 则评价结构重叠与地标对应。两类记录共同说明方法在有限回放条件下的配准表现。

\section{本章小结}
\label{sec:registration-summary}

本章提出 SAMCL，将有限历史图像对回放、一阶元持续学习与锐度感知优化结合，用于不同器官和模态的持续配准。四任务实验中，SAMCL 改善了直接顺序训练的历史保持，与 MER 的相对表现则随任务和指标变化。四项任务的性能分布，以及脑部、腹部和肺部的代表性配准结果，进一步展示了平均指标之外的结果差异。容量消融及域内、跨任务测试表明，回放规模与锐度感知更新均会影响模型表现。

本章将第三章的阶段矩阵、BWTR、RMA、MPE 和 DRR 用于持续配准。SAMCL 的 Dice-BWTR 和 Dice-RMA 分别约为 $-0.0751$ 和 $1.0954$，高于 MER 的 $-0.0784$ 和 $1.0374$；EWC 的 Dice-BWTR 更高，TRE 指标则体现 MER 的相对优势。SAMCL 的 MPE 为零，DRR 为 $0.1806$。配准实验与前两章一样，保留原始任务指标，再用阶段变化和资源记录分析持续更新的效果。
