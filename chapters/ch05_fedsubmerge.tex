...
第三章的 MedCL 平台以任务序列、阶段--任务性能矩阵和多维指标组织持续学习评价，第四章将这一设置用于稀疏涂鸦监督下的分割。本章进一步研究图像级医学分类：类别按批次到达，训练数据分布于多个客户端，后续阶段不回放历史原始样本。评价沿用 MedCL 的历史保持、当前学习与资源分析，将分割矩阵中的 Dice 换为任务准确率，报告 ACC、BWTR、RMA、MPE 和 DRR，从而把平台的评价体系扩展到无回放联邦持续学习。
...
表\ref{tab:fedsubmerge-main-distribution}报告 PathMNIST 和 Hyper-Kvasir 的分布偏斜结果，表\ref{tab:fedsubmerge-main-other}报告标签数量偏斜结果，表\ref{tab:fedsubmerge-main-feature}报告多来源皮肤病变特征偏斜结果。三表覆盖七个联邦设置，以 ACC 评价最终分类性能，并结合 MedCL 的 BWTR、RMA、MPE 和 DRR 分析历史保持、当前学习与资源使用。
...
\paragraph{模型与历史信息资源。}
在上述两个 $\alpha=0.3$ 的设置中，FedSubMerge 与 FedSubMerge-AD 的 MPE 均为零，DRR 在 PathMNIST 上为 $0.1169$，在 Hyper-Kvasir 上为 $0.9477$，均低于 FOT 的 $1.0000$。两种融合方式在同一数据设置下具有相同 DRR，其差别主要体现在服务器对客户端历史方向的选择与融合。DRR 与逐层子空间的字节量、SVD 计算及通信量共同描述资源使用，后者在开销分析中进一步讨论。
...
表\ref{tab:fedsubmerge-system-boundary}概括本章与 MedCL 评价的对应关系。主结果表将任务特定的 ACC 与平台共用的 BWTR、RMA、MPE 和 DRR 并列组织；异质性、参与率和设计消融进一步提供 BWT 与通信开销分析。性能矩阵、历史任务曲线和类别混淆结果用于解释各项汇总指标。
