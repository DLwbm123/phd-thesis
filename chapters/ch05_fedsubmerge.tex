...
第三章的 MedCL 平台以任务序列、阶段--任务性能矩阵和多维指标组织持续学习评价，第四章据此完成密集监督分割基准评测，并在固定数据与测试设置下研究涂鸦监督方法。本章进一步研究图像级医学分类：类别按批次到达，训练数据分布于多个客户端，后续阶段不回放历史原始样本。评价沿用 MedCL 的历史保持、当前学习与资源分析，将分割矩阵中的 Dice 换为任务准确率，以 ACC、BWTR 和 RMA 评价任务表现，并结合 MPE 与 DRR 分析模型扩展和历史信息使用，从而把平台的评价体系扩展到无回放联邦持续学习。
...
\begin{figure}[p]
\centering
\includegraphics[width=\textwidth,height=0.81\textheight,keepaspectratio]{ch05/fedsubmerge_overview_a_large.pdf}
\caption[FedSubMerge 与 FedSubMerge-AD 的整体流程]{FedSubMerge 与 FedSubMerge-AD 的整体流程。（a）联邦任务流与主梯度子空间构建。客户端完成轮次联邦训练后，利用最终模型采样当前任务梯度，并通过奇异值分解构建逐层主梯度子空间；服务器融合与后续本地训练见续图。}
...
\centering
\includegraphics[width=\textwidth,height=0.81\textheight,keepaspectratio]{ch05/fedsubmerge_overview_b_large.pdf}
\caption{FedSubMerge 与 FedSubMerge-AD 的整体流程（续）。（b）服务器端子空间融合。FedSubMerge 统一融合全部客户端子空间；FedSubMerge-AD 按层选择相似客户端，再对加权基进行个性化融合。}
\end{figure}
\begin{figure}[p]
\ContinuedFloat
\captionsetup{list=off}
\centering
\includegraphics[width=\textwidth,height=0.81\textheight,keepaspectratio]{ch05/fedsubmerge_overview_c_large.pdf}
\caption{FedSubMerge 与 FedSubMerge-AD 的整体流程（续）。（c）本地投影训练与客户端子空间更新。后续任务中，新梯度投影到返回子空间的正交补，并在每个任务结束时递归更新客户端主梯度子空间。}
\end{figure}
...
\begin{figure}[p]
\centering
\includegraphics[width=\linewidth,height=0.75\textwidth,keepaspectratio]{ch05/fedsubmerge_hyperparameter.pdf}
...
\begin{figure}[p]
\centering
\includegraphics[width=\linewidth,height=0.75\textwidth,keepaspectratio]{ch05/fedsubmerge_accuracy_matrix.pdf}
...
\begin{figure}[htbp]
\centering
\resizebox{\textwidth}{!}{\input{figures/ch05/fedsubmerge_engineering.tex}}
