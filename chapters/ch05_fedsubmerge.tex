...
第四章通过有限图像--涂鸦回放保留旧任务能力。本章转向另一种训练条件：医学分类数据分布在多个客户端，类别按批次到达，后续阶段不能回放历史原始样本。评价仍沿用第三章 MedCL 的阶段矩阵，但以任务准确率代替分割 Dice，用 ACC 与 BWTR 分析最终表现和历史保持，用任务矩阵的对角条目观察初学表现，并结合 MPE、DRR 及通信记录分析资源开销。
...
本章先说明研究动机与问题建模，再介绍 FedSubMerge、FedSubMerge-AD 及其局部理论分析。实验涵盖七个联邦设置，主比较报告 ACC、BWTR、MPE 和 DRR，并通过异质性、参与率和消融实验分析方法表现。最后结合 MedCL 的记录方式讨论阶段全局模型、客户端分布与资源开销。
...
\end{equation}
BWTR 衡量旧任务相对于初学性能的变化，RMA 衡量顺序约束下新任务的初学表现相对于独立训练参照的水平，两者均越高越好。本章主结果表中的 BWTR 以百分数表示，即将式\eqref{eq:fedsubmerge-bwtr-rma}的比值乘以 $100\%$；其百分数显示与异质性、参与率和消融表中以准确率百分点表示的 BWT 不同。本章以 ACC 与 BWTR 进行总体性能比较，任务初学表现通过准确率矩阵的对角条目分析。MPE 与 DRR 分别沿用式\eqref{eq:benchmark-mpe}和式\eqref{eq:benchmark-drr}，记录模型参数扩展和历史信息构建、复用所涉及的数据比例。FedSubMerge 以梯度子空间摘要保留历史信息，后续阶段通过摘要约束更新。

\subsection{总体性能}
\label{subsec:fedsubmerge-main-results}

表\ref{tab:fedsubmerge-main-distribution}报告 PathMNIST 和 Hyper-Kvasir 的分布偏斜结果，表\ref{tab:fedsubmerge-main-other}报告标签数量偏斜结果，表\ref{tab:fedsubmerge-main-feature}报告多来源皮肤病变特征偏斜结果。三表覆盖七个联邦设置，以 ACC 评价最终分类性能，以 MedCL 的 BWTR 评价历史保持，同时列示 MPE 与 DRR。空白或破折号表示相应资源记录缺失，不视为零值；当前任务学习结合任务准确率矩阵另行分析。

\input{tables/FedCL}

在七个设置中，FedSubMerge 或 FedSubMerge-AD 均取得无原始样本回放方法中的最高 ACC。与各设置中最强的无回放比较方法相比，两种方法中较高的 ACC 提升为 $1.61$--$8.61$ 个百分点。纳入回放方法后，所提方法在五个设置中取得最高 ACC；PathMNIST $\alpha=0.1$ 下，FedSubMerge-AD 的 $79.40\%$ 低于 Re-Fed 的 $80.72\%$；Hyper-Kvasir $\alpha=0.1$ 下，FedSubMerge 的 $76.64\%$ 低于 DDDR 的 $77.24\%$。所提方法在这两个设置中取得第二高 ACC。

FedSubMerge-AD 在七个设置中的五个取得高于 FedSubMerge 的 ACC，但统一融合的 BWTR 在七个设置中均更高。在所比较的无原始样本回放方法中，FedSubMerge 在七个设置中均取得最高 BWTR；纳入回放方法后，其 BWTR 在五个设置中仍为最高，Hyper-Kvasir $\alpha=0.1$ 和 OrganAMNIST 标签数量偏斜中的最高值则由 DDDR 取得。这说明自适应融合较常获得更高最终准确率，统一融合则具有更充分的平均相对历史保持，两者在最终性能与历史保持之间呈现不同取舍。

PathMNIST $\alpha=0.3$ 下，FedSubMerge 与 FedSubMerge-AD 的 ACC 分别为 $90.97\%$ 和 $93.42\%$，BWTR 分别为 $-0.94\%$ 和 $-3.51\%$。自适应融合的 ACC 高 $2.45$ 个百分点，统一融合的 BWTR 高 $2.57$ 个百分点。与 FOT 的 $86.20\%$ ACC 和 $-6.93\%$ BWTR 相比，两种融合方式均改善了最终性能与历史保持；Fed-DER 的 ACC 为 $81.70\%$、BWTR 为 $-14.24\%$，并不具有更小的相对历史下降。异质性增强至 $\alpha=0.1$ 后，两种方法的 ACC 分别降至 $77.50\%$ 和 $79.40\%$，BWTR 分别为 $-16.21\%$ 和 $-18.31\%$，历史退化明显加重。

Hyper-Kvasir $\alpha=0.3$ 下，FedSubMerge 的 ACC 和 BWTR 分别为 $78.64\%$ 和 $-7.75\%$，均高于 FedSubMerge-AD 的 $78.12\%$ 和 $-12.45\%$。与 FOT 相比，统一融合的 ACC 和 BWTR 分别提高 $8.61$ 和 $6.34$ 个百分点；与 ACC 最高的回放基线 DDDR 相比，ACC 提高 $3.37$ 个百分点。$\alpha=0.1$ 下，统一融合仍优于自适应融合，但其 ACC 和 BWTR 分别低于 DDDR $0.60$ 和 $0.34$ 个百分点。这些结果说明，相关客户端选择在 Hyper-Kvasir 的两个分布偏斜设置中没有优于统一融合。

标签数量偏斜下，OrganAMNIST 上 FedSubMerge-AD 的 ACC 为 $74.42\%$，比 FedSubMerge 的 $70.17\%$ 高 $4.25$ 个百分点，也高于 DDDR 的 $69.35\%$。但二者的 BWTR 分别为 $-18.72\%$ 和 $-18.34\%$，均低于 DDDR 的 $-15.34\%$，说明较高最终 ACC 不等于较小相对遗忘。Hyper-Kvasir 上，自适应与统一融合的 ACC 分别为 $70.97\%$ 和 $70.76\%$；统一融合的 BWTR 为 $0.10\%$，高于自适应融合的 $-2.04\%$。这一接近零的正值表示旧任务平均相对表现略有改善，不能推断每个旧任务都没有性能下降。

多来源皮肤病变特征偏斜下，FedSubMerge 与 FedSubMerge-AD 的 ACC 分别为 $85.53\%$ 和 $85.75\%$，均高于回放基线中的最高值 DDDR 的 $84.43\%$。二者的 BWTR 分别为 $-5.10\%$ 和 $-5.83\%$，也均高于表中其他方法。这一设置再次体现自适应融合在最终 ACC 上略高、统一融合在平均相对历史保持上更好的取舍。

资源方面，三张主表中给出的 MPE 值均为 $0.0000$，表明相应配置不依赖模型参数扩展。OrganAMNIST 及部分基线的资源项缺失，因此资源比较仅覆盖有记录的配置。

PathMNIST 的 $\alpha=0.3$ 和 $\alpha=0.1$ 设置中，FedSubMerge 与 FedSubMerge-AD 的 DRR 分别为 $0.1169$ 和 $0.0956$，均高于相应 Fed-GPM 的 $0.0049$ 和 $0.0039$。Hyper-Kvasir 两个分布偏斜设置中，两种融合方法的 DRR 分别为 $0.9477$ 和 $0.8638$，也高于 Fed-GPM 的 $0.2393$ 和 $0.1735$。在两个 $\alpha=0.3$ 设置中，它们的 DRR 均低于 FOT 的 $1.0000$；PathMNIST 上则高于 Fed-DER 的 $0.0749$，Hyper-Kvasir 上高于 Fed-DER 的 $0.4473$。因此，无原始样本回放不意味着用于构建历史表示的数据比例最低。

Hyper-Kvasir 标签数量偏斜中，两种融合方法的 DRR 均为 $1.0000$，Fed-GPM 为 $0.0868$；多来源皮肤病变中，两种融合方法均为 $0.2485$，Fed-GPM 为 $0.0096$。相同设置下，两种融合方式的 DRR 相同，但服务器返回的子空间及其实际秩可能不同。按照第三章定义，DRR 计入构造历史表示所涉及的数据，并不等同于后续回放原始图像的比例，也不能直接表示摘要字节量、SVD 计算或通信量。这些开销在后文分别分析。

综合各设置，ACC 与 BWTR 分别刻画最终分类表现和相对历史变化，不总是给出相同排序。任务矩阵的对角条目进一步反映初学表现。所有分类结果均在表\ref{tab:fedsubmerge-setup}规定的类别批次和测试任务身份已知的条件下获得。
...
\caption[联邦持续分类与 MedCL 的评价流程]{联邦持续分类与 MedCL 的评价流程。训练程序生成阶段全局模型，任务适配器按测试任务选择分类头，阶段矩阵用于汇总 ACC 与 BWTR，独立训练参照齐备时可进一步计算 RMA，训练配置用于记录 MPE、DRR 与通信开销。}
...
表\ref{tab:fedsubmerge-system-boundary}概括本章与 MedCL 评价的对应关系。主结果表将任务特定的 ACC 与平台共用的 BWTR 并列组织，以 MPE 与 DRR 分别分析模型扩展和历史信息使用；异质性、参与率和设计消融进一步提供 BWT 与通信开销分析。性能矩阵、历史任务曲线和类别混淆结果用于解释各项汇总指标。
...
在七个联邦设置中，FedSubMerge 与 FedSubMerge-AD 中表现较好的版本均取得无回放比较方法中的最高 ACC。MedCL 指标进一步揭示最终性能与历史保持的不同取舍：PathMNIST $\alpha=0.3$ 上，FedSubMerge-AD 的 ACC 和 BWTR 分别为 $93.42\%$ 和 $-3.51\%$，统一融合的 ACC 为 $90.97\%$、BWTR 为 $-0.94\%$；Hyper-Kvasir $\alpha=0.3$ 上，FedSubMerge 的 ACC 和 BWTR 分别为 $78.64\%$ 和 $-7.75\%$，均高于自适应版本。在七个设置中，统一融合的 BWTR 均高于自适应融合，而自适应融合在五个设置中具有更高 ACC。完全参与时，阶段末额外 PGS 通信约占 20 个模型通信轮次的 $3.0\%$--$3.5\%$。下一章沿用平台评价，研究有限图像对回放下的持续配准。
