...
第四章通过有限图像--涂鸦回放保留旧任务能力。本章转向另一种训练条件：医学分类数据分布在多个客户端，类别按批次到达，后续阶段不能回放历史原始样本。评价仍沿用第三章 MedCL 的阶段矩阵，但以任务准确率代替分割 Dice，用 ACC 与 BWTR 分析最终表现和历史保持，用任务矩阵的对角条目观察初学表现，并结合历史摘要的保存形式和通信记录分析资源开销。
...
联邦模型既要保留本地旧任务知识，也要保留从其他客户端获得的知识。客户端只用当前数据更新时，单独保存的历史信息无法覆盖其他中心的经历；服务器若将所有历史方向都用于约束每个客户端，又可能压缩新任务的学习空间。本章因此研究如何融合跨客户端历史信息，以及如何减少不相关方向对当前学习的限制。
...
本章先说明研究动机与问题建模，再介绍 FedSubMerge、FedSubMerge-AD 及其局部理论分析。实验涵盖七个联邦设置，主比较报告 ACC 与 BWTR，并通过异质性、参与率和消融实验分析方法表现。最后结合 MedCL 的记录方式讨论阶段全局模型、客户端分布与资源开销。

\section{方法动机与问题建模}
\subsection{方法动机}
\label{sec:fedsubmerge-motivation}

已有联邦持续学习方法采用不同形式的历史信息。例如，Ke 等的 TPPR 结合视觉提示、任务感知的提示梯度投影和提示回放，研究参数高效的联邦类别增量学习\cite{ke2025tppr}。其保护对象是预训练模型中的少量可训练参数，本章则考察如何保存和融合样本级梯度子空间。

本章的训练信息限制来自两个方面：不同机构只能在本地使用原始图像，服务器仅接收模型参数或其他允许传输的信息；进入新任务后，客户端也不能再读取旧任务原始样本。常规联邦学习侧重同一阶段的跨客户端聚合，单机持续学习侧重一个模型的顺序更新。两者结合时，当前本地梯度无法完整反映模型此前从其他机构获得的知识。
...
\begin{figure}[htbp]
\centering
\thesisgraphic{ch05/fedsubmerge_overview.pdf}
\caption[FedSubMerge 与 FedSubMerge-AD 的整体流程]{FedSubMerge 与 FedSubMerge-AD 的整体流程。（a）联邦任务流与客户端主梯度子空间构建；（b）服务器端统一融合或按层选择相关客户端后进行自适应融合；（c）将后续本地梯度投影到保护空间的正交补，并在任务结束时更新客户端子空间。}
...
任务 $t$ 开始前，客户端接收服务器在任务 $t-1$ 后返回的共享或个性化 PGS，并在本地训练时将共享特征提取器的梯度投影到其正交补。任务结束后，客户端将此前返回的加权基与当前任务梯度共同压缩为新的 PGS。服务器再进行统一或自适应融合，为下一任务提供保护空间。首个任务没有历史 PGS，采用常规本地更新。
...
\end{equation}
这些摘要不包含原始图像、标签、未归一化输出（logits）、生成样本或回放样本集合。该存储形式支持无原始样本回放，但不能据此推导形式化隐私性质。
...
\begin{table}[htbp]
\centering
\caption{FedSubMerge 实验数据、联邦划分与任务流设置。}
...
第三章 MedCL 的 BWTR 与 RMA 分别评价历史保持与当前任务学习。在分类任务中，二者以准确率作为基础指标。令 $a_{\tau}^{NC}$ 表示任务 $\tau$ 独立训练参照的准确率，在式\eqref{eq:benchmark-bwtr}和式\eqref{eq:benchmark-rma}中以准确率作为任务指标，得到
\begin{equation}
\mathrm{BWTR}
=
\frac{1}{T-1}\sum_{\tau=1}^{T-1}
...
\end{equation}
BWTR 衡量旧任务相对于初学性能的变化，RMA 衡量新任务的初学表现相对于独立训练参照的水平，两者均越高越好。本章主结果表中的 BWTR 以百分数表示，即将式\eqref{eq:fedsubmerge-bwtr-rma}的比值乘以 $100\%$；它不同于异质性、参与率和消融表中以准确率百分点表示的 BWT。本章以 ACC 与 BWTR 进行总体比较，任务初学表现通过准确率矩阵的对角条目分析。RMA 需要相应的逐任务独立训练参照，主结果表不报告该项数值。历史信息访问方式、子空间存储和通信开销结合方法配置另行讨论。

\subsection{总体性能}
\label{subsec:fedsubmerge-main-results}

表\ref{tab:fedsubmerge-main-distribution}报告 PathMNIST 和 Hyper-Kvasir 的分布偏斜结果，表\ref{tab:fedsubmerge-main-other}报告标签数量偏斜结果，表\ref{tab:fedsubmerge-main-feature}报告多来源皮肤病变特征偏斜结果。三表覆盖七个联邦设置，以 ACC 评价最终分类性能，以 BWTR 评价旧任务相对于初学表现的平均变化。表中星号用于区分使用存储或生成历史样本的回放方法，不表示显著性检验。

\input{tables/FedCL}

在七个设置中，FedSubMerge 与 FedSubMerge-AD 中 ACC 较高的版本均超过无原始样本回放基线；相较各设置中 ACC 最高的无回放基线，提升幅度为 $1.61$--$8.61$ 个百分点。纳入回放方法后，所提方法在五个设置中取得最高 ACC，在其余两个设置中取得第二高 ACC。具体而言，PathMNIST $\alpha=0.1$ 下，FedSubMerge-AD 的 $79.40\%$ 低于 Re-Fed 的 $80.72\%$；Hyper-Kvasir $\alpha=0.1$ 下，FedSubMerge 的 $76.64\%$ 低于 DDDR 的 $77.24\%$。因此，无原始样本回放条件下的优势不等于在全部比较方法中始终最优。

两种融合方式的排序随评价指标变化。FedSubMerge-AD 在七个设置中的五个取得更高 ACC，而 FedSubMerge 的 BWTR 在七个设置中均高于自适应版本。与无原始样本回放基线比较，FedSubMerge 在七个设置中均取得最高 BWTR；纳入回放方法后，仍在五个设置中最高，例外为 Hyper-Kvasir $\alpha=0.1$ 和 OrganAMNIST 标签数量偏斜，两者的最高 BWTR 均由 DDDR 取得。总体上，自适应融合更常提高最终准确率，统一融合在本组实验中具有更好的平均相对历史保持，但两者都不能保证每个旧任务的性能不下降。

PathMNIST $\alpha=0.3$ 下，FedSubMerge 与 FedSubMerge-AD 的 ACC 分别为 $90.97\%$ 和 $93.42\%$，BWTR 分别为 $-0.94\%$ 和 $-3.51\%$。自适应融合的 ACC 高 $2.45$ 个百分点，统一融合的 BWTR 高 $2.57$ 个百分点。两种版本在这两项指标上均优于无回放基线 FOT 的 $86.20\%$ 和 $-6.93\%$。异质性增强至 $\alpha=0.1$ 后，两种版本的 ACC 分别降至 $77.50\%$ 和 $79.40\%$，BWTR 降至 $-16.21\%$ 和 $-18.31\%$，表明更强的类别比例差异同时影响最终表现和历史保持。此时 Re-Fed 的 ACC 最高，但 FedSubMerge 的 BWTR 仍高于 Re-Fed 的 $-16.70\%$，再次体现两项指标的排序差异。

Hyper-Kvasir 的两个分布偏斜设置均更有利于统一融合。$\alpha=0.3$ 下，FedSubMerge 的 ACC 和 BWTR 为 $78.64\%$ 和 $-7.75\%$，分别高于 FedSubMerge-AD $0.52$ 和 $4.70$ 个百分点；相较 FOT，两项指标分别提高 $8.61$ 和 $6.34$ 个百分点，相较 ACC 最高的回放基线 DDDR，ACC 提高 $3.37$ 个百分点。$\alpha=0.1$ 下，FedSubMerge 的 ACC 和 BWTR 为 $76.64\%$ 和 $-12.25\%$，仍高于自适应版本的 $76.16\%$ 和 $-14.85\%$，但分别低于 DDDR $0.60$ 和 $0.34$ 个百分点。因此，相关客户端选择并未在这两个设置中带来更高的最终性能或平均历史保持。

标签数量偏斜下，OrganAMNIST 上 FedSubMerge-AD 的 ACC 为 $74.42\%$，比 FedSubMerge 的 $70.17\%$ 高 $4.25$ 个百分点，也比 DDDR 的 $69.35\%$ 高 $5.07$ 个百分点。但自适应与统一融合的 BWTR 分别为 $-18.72\%$ 和 $-18.34\%$，均低于 DDDR 的 $-15.34\%$。较高的最终 ACC 并未对应较小的相对遗忘，不能仅凭 ACC 判断历史保持能力。

Hyper-Kvasir 标签数量偏斜下，FedSubMerge-AD 与 FedSubMerge 的 ACC 分别为 $70.97\%$ 和 $70.76\%$，仅相差 $0.21$ 个百分点；BWTR 则分别为 $-2.04\%$ 和 $0.10\%$。统一融合的 BWTR 高于所有比较方法，自适应融合则低于 Re-Fed 的 $-1.76\%$。统一融合接近零的正 BWTR 表示旧任务的平均相对表现略有提高，但仍可能存在个别旧任务下降。

多来源皮肤病变特征偏斜下，FedSubMerge 与 FedSubMerge-AD 的 ACC 分别为 $85.53\%$ 和 $85.75\%$，比 ACC 最高的回放基线 DDDR 分别高 $1.10$ 和 $1.32$ 个百分点。二者的 BWTR 分别为 $-5.10\%$ 和 $-5.83\%$，也均高于表中其他方法。自适应融合的最终 ACC 略高，统一融合的平均相对历史保持更好；这一结果与 PathMNIST 上观察到的取舍一致。

上述性能比较仍需结合历史访问条件理解。FedSubMerge 的两个版本保存梯度子空间摘要；Fed-DER、GLFC 和 Re-Fed 保存历史样本，TARGET 与 DDDR 使用生成样本。无原始样本回放并不意味着没有历史存储或附加计算。子空间摘要的大小与逐层保留秩有关，其构建、融合和传输开销在第\ref{subsec:fedsubmerge-efficiency}节讨论，客户端规模与参与率下的通信结果见表\ref{tab:fedsubmerge-scalability}。

综合七个设置，ACC 与 BWTR 分别刻画最终分类表现和相对历史变化，不能相互替代。任务矩阵的对角条目进一步描述初学表现，但没有独立训练参照时，不能将较高的对角均值等同于较高 RMA。所有结果均在表\ref{tab:fedsubmerge-setup}规定的类别批次和测试任务身份已知的条件下获得，不能直接外推到任务身份未知的累计类别预测。

\subsection{异质性、客户端规模与参与率}
\label{subsec:fedsubmerge-robustness}

表\ref{tab:fedsubmerge-heterogeneity}考察 PathMNIST 上不同 Dirichlet 参数。$\alpha$ 越小，客户端类别比例越不均衡。四种方法的 ACC 均随异质性增强而下降。BWT 的总体趋势相似，但并非逐点单调：FedSubMerge 在 $\alpha=0.5$ 时为 $-1.05$，在 $\alpha=0.3$ 时为 $-0.86$ 个百分点。在所比较的 $\alpha$ 和 IID 设置下，FedSubMerge-AD 的 ACC 最高，FedSubMerge 的 BWT 最高。例如，$\alpha=0.05$ 时，前者的 ACC 为 $75.20\%$，后者的 BWT 为 $-19.30$ 个百分点。
...
表\ref{tab:fedsubmerge-scalability}在 PathMNIST $\alpha=0.3$、$n=4$ 下改变客户端总数和参与率。固定客户端规模时，参与率由 $1.00$ 降至 $0.30$，ACC 与 BWT 均下降。以 $K=40$ 为例，ACC 从 $90.20\%$ 降至 $84.85\%$，BWT 从 $-6.95$ 变为 $-12.40$ 个百分点。固定参与率时，三个客户端规模的 ACC 随 $K$ 增大而下降，但 BWT 并非处处单调，例如参与率 $0.30$ 下，$K=20$ 的 $-8.17$ 略高于 $K=10$ 的 $-8.25$ 个百分点。由于相关客户端数 $n$ 固定，该比较同时涉及客户端规模、参与情况和相关客户端所占比例的变化。
...
完全参与时，PGS 额外通信约占 20 个模型通信轮次的 $3.0\%$--$3.5\%$；参与率为 $0.50$ 时，该比例为 $5.9\%$--$6.4\%$，降至 $0.30$ 时为 $11.8\%$--$12.3\%$。该比例以相应设置下的模型通信量为分母，表示任务结束时的额外子空间通信，不是每个常规轮次新增的传输比例。

\subsection{消融、参数敏感性与任务级结果}
\label{subsec:fedsubmerge-ablation}

表\ref{tab:fedsubmerge-ablation}比较 PathMNIST $\alpha=0.3$ 上的主要设计。仅加入本地 PGS 后，ACC 从 FedAvg 的 $67.23\%$ 提高到 $82.05\%$，BWT 从 $-25.70$ 改善为 $-17.60$ 个百分点。进一步采用跨客户端融合，FedSubMerge 和 FedSubMerge-AD 的 ACC 分别为 $90.97\%$ 和 $93.42\%$，比仅使用本地 PGS 分别高 $8.92$ 和 $11.37$ 个百分点；BWT 也分别提高到 $-0.86$ 和 $-3.37$ 个百分点。该消融表明，在本地保护之外融合其他客户端的历史方向，有助于同时改善最终准确率与历史任务保持。
...
完整 FedSubMerge-AD 相较基于激活的子空间配置，ACC 高 $5.62$ 个百分点，BWT 高 $3.86$ 个百分点。相较随机选择，按子空间相似性选择客户端使最终 ACC 提高 $4.87$ 个百分点，但 BWT 低 $0.13$ 个百分点，两项指标呈现不同变化方向。去除奇异值加权后，ACC 和 BWT 均下降，说明加权融合在这一设置下有利于最终性能和历史保持。统一融合相较自适应融合，BWT 高 2.51 个百分点，ACC 低 2.45 个百分点。

图\ref{fig:fedsubmerge-hyperparameter}给出多来源皮肤病变设置上的相关客户端数和服务器能量阈值敏感性。$K=4$ 时，$n=0$ 仅使用目标客户端 PGS，跨机构共享不足；$n=3$ 使用全部客户端，与统一融合对应，BWT 略有改善但 ACC 下降；$n=2$ 在两项指标之间取得本实验采用的平衡。提高 $\gamma$ 通常保留更多旧方向并改善 BWT，也会增加投影约束。$\gamma=0.97$ 获得最高 ACC，因而与 $n=2$ 共同作为默认配置。该默认配置根据本实验的验证设置确定。

\begin{figure}[htbp]
\centering
\thesisgraphic{ch05/fedsubmerge_hyperparameter.pdf}
\caption{多来源皮肤病变特征偏斜设置上的 FedSubMerge-AD 参数敏感性。（a）相关客户端数 $n$；当 $K=4$ 时，$n=0$ 只使用目标客户端 PGS，$n=3$ 使用全部其他客户端。（b）服务器能量阈值 $\gamma$。ACC 以百分数、BWT 以百分点报告。星号表示默认配置 $n=2$、$\gamma=0.97$。}
...
\begin{figure}[htbp]
\centering
\thesisgraphic{ch05/fedsubmerge_accuracy_matrix.pdf}
...
\subsection{平台评价与方法讨论}
\label{sec:fedsubmerge-discussion}
\label{sec:fedsubmerge-engineering}
\label{sec:fedsubmerge-platform-integration}

图\ref{fig:fedsubmerge-engineering}说明联邦训练与 MedCL 评价的衔接。客户端更新、模型聚合和阶段末子空间融合由训练程序完成，平台接收各类别阶段结束后的全局模型或分类预测。任务配置保存数据来源、类别批次、顺序、阶段和分类头映射；模型接口读取 ResNet-18 权重，预测接口读取与测试样本对应的类别概率或标签。

阶段 $t$ 的全局模型在任务 $j$ 上测试时，使用任务 $j$ 的分类头和类别映射，得到 $a_{t,j}$。这些条目按式\eqref{eq:fedsubmerge-acc-bwt}和式\eqref{eq:fedsubmerge-bwtr-rma}计算 ACC、BWT 与 BWTR；具备独立训练参照时，还可计算 RMA。模型参数、历史信息使用及模型和 PGS 传输记录分别补充资源分析。

\begin{figure}[htbp]
\centering
\resizebox{\thesisfigurewidth}{!}{\input{figures/ch05/fedsubmerge_engineering.tex}}
\caption[联邦持续分类与 MedCL 的评价流程]{联邦持续分类与 MedCL 的评价流程。训练程序生成阶段全局模型，任务适配器按测试任务选择分类头，阶段矩阵用于汇总 ACC 与 BWTR，独立训练参照齐备时可进一步计算 RMA，训练配置另行记录历史摘要形式及模型和子空间通信开销。}
...
表\ref{tab:fedsubmerge-system-boundary}概括本章与 MedCL 评价的对应关系。主结果表将任务特定的 ACC 与平台共用的 BWTR 并列组织，异质性、参与率和设计消融进一步提供 BWT 与通信开销分析。任务准确率矩阵用于区分初学表现和后续保持，历史摘要的保存形式及其实际秩用于说明方法的存储与计算需求。
...
保持与学习 & 历史任务与初学性能；独立训练参照用于 RMA & BWTR 与阶段矩阵；具备独立训练参照时计算 RMA \\
客户端分布 & 分布、标签数量与来源特征偏斜 & 联邦设置、样本分组与分布记录 \\
资源使用 & 历史摘要、逐层保留秩及模型与 PGS 传输 & 摘要保存形式与额外通信开销 \\
\bottomrule
\end{tabularx}
\end{table}

从方法效果看，仅使用本地 PGS 与完整方法的消融比较表明，加入其他客户端的历史方向有助于提高最终准确率。子空间基与奇异值因而能够在不保留历史原始图像的条件下，为后续更新提供历史约束。

统一融合与自适应融合的取舍随数据设置变化。PathMNIST $\alpha=0.3$ 上，自适应融合具有更高的 ACC，统一融合的 BWTR 更高；Hyper-Kvasir $\alpha=0.3$ 上，统一融合的 ACC 与 BWTR 均更高。这一差异说明客户端相关性、历史方向覆盖和当前任务学习需要联合分析，而不能由单一指标决定融合方式。
...
在七个联邦设置中，两种版本中 ACC 较高者均超过无原始样本回放基线；纳入回放方法后，在五个设置中最高、两个设置中次高。FedSubMerge-AD 在五个设置中具有更高 ACC，FedSubMerge 的 BWTR 则在七个设置中均高于自适应版本。任务矩阵和消融结果进一步区分了初学表现、历史保护及跨客户端融合的作用。完全参与时，阶段末额外 PGS 通信约占 20 个模型通信轮次的 $3.0\%$--$3.5\%$；无回放条件下仍需承担子空间摘要的存储、构建和传输开销。下一章研究有限图像对回放下的持续配准。