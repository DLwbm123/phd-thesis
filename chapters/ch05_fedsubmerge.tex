...
第三章建立了 MedCL 评测框架，第四章研究跨中心弱监督持续分割。本章研究图像级医学分类：类别按批次到达，训练数据分布于多个客户端，后续阶段不回放历史原始样本。上述条件使模型既要学习新增类别，也要保持其他客户端参与学习的历史类别知识。

本章将每批新增类别组织为一个任务。测试时已知样本所属任务，模型使用该任务的分类头进行预测，候选类别限于对应类别批次。因此，本章采用任务身份已知的多头分类设置，不要求模型在全部已见类别中判断任务归属。
...
本章依次介绍问题建模、FedSubMerge 与 FedSubMerge-AD、局部理论分析和七个联邦设置下的实验结果。随后设计将阶段全局模型或预测接入 MedCL 的评测流程，在保持分类头选择方式的基础上讨论客户端分布与任务顺序的联合展示。
...
\end{equation}
ACC 衡量最终模型在各已见任务上的平均准确率。BWT 为旧任务准确率从初学完成到最终阶段的平均变化：负值表示平均下降，零表示平均变化为零，正值表示平均提高，因此 BWT 越高越好。下文 ACC 以百分数报告，BWT 以百分点报告；表中数值不作缩放。

\subsection{总体性能}
\label{subsec:fedsubmerge-main-results}

表\ref{tab:fedsubmerge-main-distribution}报告分布偏斜结果，表\ref{tab:fedsubmerge-main-other}报告标签数量偏斜和多来源特征偏斜结果。两表共覆盖七个设置，比较最终 ACC 与 BWT；星号标记使用存储或生成历史样本的回放方法。

\begin{table}[htbp]
\centering
\caption{PathMNIST 与 Hyper-Kvasir 分布偏斜设置上的结果。ACC 以百分数、BWT 以百分点报告，二者均越高越好。红色粗体表示列最优值；$^{\ast}$ 表示回放方法。}
...
\begin{table}[htbp]
\centering
\caption{标签数量偏斜与多来源特征偏斜设置上的结果。ACC 以百分数、BWT 以百分点报告，二者均越高越好。红色粗体表示列最优值；$^{\ast}$ 表示回放方法。}
...
FedSubMerge-AD 在七个设置中的五个取得高于 FedSubMerge 的 ACC。PathMNIST $\alpha=0.3$ 上，两者分别为 $93.42\%$ 和 $90.97\%$；FedSubMerge-AD 在 OrganAMNIST 标签数量偏斜和皮肤病变特征偏斜设置上分别达到 $74.42\%$ 和 $85.75\%$。这些结果表明自适应融合在上述设置下取得了更高的最终平均准确率。

FedSubMerge 在七个设置中的 BWT 均高于 FedSubMerge-AD，并在其中五个取得全部比较方法中的最高 BWT。Hyper-Kvasir $\alpha=0.3$ 上，两者的 BWT 分别为 $-6.55$ 和 $-10.95$ 个百分点。结合 ACC 可以看出，统一融合的平均历史下降较小，自适应融合则在多数设置中取得更高最终准确率。

实验涉及病理图像、腹部 CT、消化道内镜和多来源皮肤病变图像，涵盖不同类别数、任务长度和客户端分布。各设置均按表\ref{tab:fedsubmerge-setup}组织类别批次，并在测试时提供任务身份。
...
图\ref{fig:fedsubmerge-accuracy-matrix}展示 Hyper-Kvasir $\alpha=0.3$ 上两种方法的任务准确率矩阵。FedSubMerge 与 FedSubMerge-AD 的十个对角条目平均值分别为 $84.56\%$ 和 $87.95\%$，即自适应方法的平均初学准确率较高。主表中两者的 BWT 分别为 $-6.55$ 和 $-10.95$ 个百分点。该设置下，自适应融合取得了更高的平均初学准确率，同时出现了更大的平均历史下降。
...
持久化存储保留客户端最近 PGS 及服务器返回 PGS 的基与奇异值，不保存回放图像或生成样本。摘要大小由各层维度和保留秩决定；随着任务增加，奇异值谱和保留秩可能变化，因此存储量不一定保持不变。更长任务流和更大主干上的实际秩、构建时间及通信量尚未测量。
...
评测阶段 $t$ 的全局模型在任务 $j$ 上推理时，选择任务 $j$ 对应的分类头，并按该任务的类别映射解释输出。最终模型或预测用于计算各任务准确率和 ACC；BWT 还需要各旧任务初学完成时的结果。缺少相应阶段结果时，仅报告最终性能，并将 BWT 标记为不可计算。

\begin{figure}[htbp]
\centering
\input{figures/ch05/fedsubmerge_engineering.tex}
\caption[联邦持续分类的模型或预测与平台评测流程]{联邦持续分类的模型或预测与平台评测流程。联邦训练与子空间聚合在平台外完成，MedCL 拟接收阶段全局模型或预测，在固定任务和客户端测试划分上评分。此图为评测接入设计，原方法框架见图\ref{fig:fedsubmerge-overview}。}
...
原实验先划分训练、验证和测试数据，再按设定的异质性分配训练客户端；多来源皮肤病变实验分别合并各来源的验证集和测试集，用于全局模型评价。原表报告的是全局任务结果。后续按来源或固定规则开展分客户端测试时，需保存每个测试样本的分组标识，重新评分并注明分组方式。
...
结果页面拟按任务、阶段和客户端展示准确率矩阵、历史任务曲线及类别混淆结果。既有论文结果与重新评分结果分别保存。参与率、本地训练周期和 PGS 传输量来自训练记录或实验核算，不由模型预测推算。
...
FedSubMerge 的 BWT 在七个设置中均高于 FedSubMerge-AD，后者则在其中五个设置取得更高 ACC。Hyper-Kvasir 的阶段矩阵进一步显示，自适应融合的平均初学准确率较高，而后续平均下降更大。两种融合方式对当前学习与历史保持的侧重不同。

实验涵盖病理图像、腹部 CT、消化道内镜和多来源皮肤病变图像，采用同步类别批次、预先给定的阶段划分、已知测试任务身份及从头训练的 ResNet-18。客户端由公开数据构造，尚未在真实多医院训练系统中验证。
...
在七个联邦设置中，FedSubMerge 与 FedSubMerge-AD 中表现较好的版本均取得无回放比较方法中的最高 ACC。统一融合的 BWT 在七个设置中均高于自适应融合，自适应融合在五个设置中取得更高 ACC。完全参与时，任务结束后的额外 PGS 通信约占 20 个模型通信轮次的 $3.0\%$--$3.5\%$。本章同时给出阶段全局模型或预测的 MedCL 接入设计。
