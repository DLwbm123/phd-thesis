...
第三章建立了对知识保持、当前任务学习、前向泛化和资源条件分别评价的思路，第四章据此研究了有限历史回放下的分割与配准。本章进一步收紧数据访问条件：历史原始样本不再允许回放，不同客户端的原始影像也不能集中，而新的分类任务仍然按顺序到达。此时，知识保护既要跨越时间，也要覆盖其他客户端曾经学习的任务，单个学习器保存少量旧样本的方案不再适用。本文据此研究无回放联邦持续医学图像分类（replay-free federated continual learning，replay-free FCL）\cite{mcmahan2017fedavg,rieke2020future,yoon2021fedweit}。本章继承第三章的评价原则，但分类数据、任务身份和准确率指标单独定义，不直接复用分割的性能排名。
...
\begin{figure}[htbp]
\centering
\input{figures/ch05/fedsubmerge_overview_zh.tex}
...
图中“原始数据不上传”表示本章设定不直接传输医学图像和回放样本。PGS 仍由模型梯度构成，因此本章不将子空间交换视为形式化隐私机制。
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.72\textwidth]{ch05/fedsubmerge_hyperparameter.pdf}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.72\textwidth]{ch05/fedsubmerge_accuracy_matrix.pdf}
