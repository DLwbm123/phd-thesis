...
第四章通过有限图像--涂鸦回放保留旧任务能力。本章转向另一种训练条件：医学分类数据分布在多个客户端，类别按批次到达，后续阶段不能回放历史原始样本。评价仍沿用第三章 MedCL 的阶段矩阵，但以任务准确率代替分割 Dice，用 ACC、BWTR 和 RMA 分别分析最终表现、历史保持与当前学习，并结合 MPE、DRR 及通信记录说明资源开销。
...
本章先说明研究动机与问题建模，再介绍 FedSubMerge、FedSubMerge-AD 及其局部理论分析。实验涵盖七个联邦设置，主比较使用 ACC、BWTR 和 RMA，并通过异质性、参与率和消融实验分析方法表现。最后结合 MedCL 的记录方式讨论阶段全局模型、客户端分布与资源开销。
...
为与第三章 MedCL 的综合评价衔接，主比较进一步使用 BWTR 和 RMA。令 $a_{\tau}^{NC}$ 表示任务 $\tau$ 独立训练参照的准确率，在式\eqref{eq:benchmark-bwtr}和式\eqref{eq:benchmark-rma}中以准确率作为任务指标，得到
\begin{equation}
\mathrm{BWTR}
=
\frac{1}{T-1}\sum_{\tau=1}^{T-1}
...
从历史保持与当前任务学习看，PathMNIST $\alpha=0.3$ 上，FedSubMerge-AD 的 BWTR 为 $-0.2288$，高于 FedSubMerge 的 $-0.2818$；其 RMA 为 $0.9886$，略低于统一融合的 $1.0017$。相较 FedAvg 的 BWTR $-0.4086$ 和 FOT 的 $-0.6127$，两种子空间融合方法均具有更小的平均相对下降。Fed-DER 的 BWTR 为 $-0.0489$，其 ACC 为 $81.70\%$，表明较小的历史相对下降与较高的最终分类准确率反映不同方面的能力。
...
在模型资源方面，两个 $\alpha=0.3$ 设置中，FedAvg、Fed-GPM、FOT、FedSubMerge 和 FedSubMerge-AD 的 MPE 均为 0；PathMNIST 中的 Fed-DER 和 Hyper-Kvasir 中的 TARGET 的 MPE 也均为 0。这些方法在相应设置中的性能变化不依赖模型参数扩展。
...
综合各设置，ACC 较高不一定意味着 BWTR 或 RMA 也最高。分析最终分类表现时，需要同时看旧任务下降、当前任务学习及所使用的历史信息。病理、腹部 CT、内镜和皮肤病变数据按表\ref{tab:fedsubmerge-setup}组织类别批次，所有结果均在测试任务身份已知的条件下获得。
...
阶段 $t$ 的全局模型在任务 $j$ 上测试时，使用任务 $j$ 的分类头和类别映射，得到 $a_{t,j}$。这些条目按式\eqref{eq:fedsubmerge-acc-bwt}和式\eqref{eq:fedsubmerge-bwtr-rma}计算 ACC、BWT、BWTR 与 RMA。模型参数、历史信息使用及模型和 PGS 传输记录分别补充资源分析。
...
在七个联邦设置中，FedSubMerge 与 FedSubMerge-AD 中表现较好的版本均取得无回放比较方法中的最高 ACC。MedCL 指标进一步揭示两种融合方式对历史保持与当前学习的不同影响：PathMNIST $\alpha=0.3$ 上，FedSubMerge-AD 的 ACC 和 BWTR 分别为 $93.42\%$ 和 $-0.2288$；Hyper-Kvasir $\alpha=0.3$ 上，FedSubMerge 的 ACC、BWTR 和 RMA 分别为 $78.64\%$、$-0.1997$ 和 $0.9982$。完全参与时，阶段末额外 PGS 通信约占 20 个模型通信轮次的 $3.0\%$--$3.5\%$。下一章沿用平台评价，研究有限图像对回放下的持续配准。
