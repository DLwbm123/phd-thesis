...
- 已合并作者刚在 Overleaf 更新的成果页内容，并按最新封面要求调整模板：右上角不再显示“学号”，仅保留“学校代码”；非盲审封面的个人信息区新增“编号：21110980022”，并紧邻置于“院系”上方。编号仍由 `main.tex` 的 `\stuid{21110980022}` 提供，但该命令不再生成右上角学号。学术与专业学位封面均适用此规则；当前专业学位封面仍采用模板原始“专业学位类别（领域）”字段与 10.5em 对齐布局。完整 XeLaTeX/BibTeX 构建为 159 页（PDF SHA-256 `7f427c2e1587d10d1c4593786bebb32461bf770585f0d6e7562d923c1000b439`），封面已视觉核验，最终日志无未解析引用/交叉引用、LaTeX error 或 Overfull 警告。
- 已合并作者在 Overleaf 对“在学期间学术成果与科研情况”的最新编辑，并按 FDSDSthesis v1.1 原模板在空“致谢”后加入 `SRC/statement.tex`。该声明页包含“复旦大学学位论文独创性声明”和“学位论文使用授权声明”，其源码与模板文件逐字节一致，签名与日期栏保持空白，且页面不含页眉页脚。完整 XeLaTeX/BibTeX 构建为 159 页（PDF SHA-256 `a5dee7091d793a986a0faa66302a4bfddb7c0cf8e5fa74d4306558e8524ee93d`）；最终 `main.log` 无未解析引用/交叉引用、LaTeX error 或 Overfull 警告，声明页已视觉核验。
- 已按作者指示调整论文末尾结构为“参考文献 → 在学期间学术成果与科研情况 → 致谢”。`\appendix` 已移除，成果章节改为无编号后置章节，目录与页面均不再显示“附录”或“第七章”；原成果条目内容保持不变。新增的“致谢”为仅含标题的空页。完整 XeLaTeX/BibTeX 构建为 159 页（PDF SHA-256 `924d9ce69b8a7fef109b7234b5d27e8c42e5f6f5dc689ba55191c80b5eb4d810`），无未解析引用/交叉引用、LaTeX error 或 Overfull 警告，末尾各页已视觉核验。
...
- 已按作者指示更新封面信息：`student-id` 设为 `21110980022`，学位类型从 `academic` 改为 `professional`，模板相应将封面显示为“博士学位论文（专业学位）”。同时按作者要求覆盖模板的专业学位封面标签逻辑，使“院系”下方字段保持为“专业：电子信息”而非“专业学位类别（领域）”。`MANIFEST.sha256` 中 `config/thesis_info.tex` 与 `fduthesis.cls` 的哈希已同步更新且全量清单校验通过。XeLaTeX/BibTeX 完整构建仍为 143 页（PDF SHA-256 `93ca95552db719cc71439d944a96f6dc3a9733342b4094ad86035321f8283a29`），最终日志无未定义引用/交叉引用、缺失文件、重复标签或 LaTeX 错误；已视觉核验中文封面信息和版式。其余论文内容、章节状态、证据与实验材料均未改动。
