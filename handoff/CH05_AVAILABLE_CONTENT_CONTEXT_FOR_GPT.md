...
- 5.3.1 是文献 `zhang2026zscribbleseg` 的静态弱监督机制和原始实验，作为 ScribbleCL 当前任务模块的引用基础；不得改写为原生持续学习方法或本文创新。前列腺结果采用 Table 9 和算术一致的 `0.706`，不采用相邻正文的 `0.726`。
- 5.3.2 只写入已审计的 MMWHS 三阶段协议、ScribbleCL 配对方法、无历史原始图像回放边界与分析公式。`experiments/scribblecl/reports/core_results.md` 明确结果仍在运行，故没有任何 ScribbleCL 数值或方向性结论。
