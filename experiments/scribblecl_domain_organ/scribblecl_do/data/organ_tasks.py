...
from .protocols import ORGAN_TASKS, TaskSpec


def h5_path(data_root: str | Path, task: TaskSpec) -> Path:
    if task not in ORGAN_TASKS:
        raise ValueError("not an Organ-CL task")
    return Path(data_root) / "Task_incre" / task.h5_name
