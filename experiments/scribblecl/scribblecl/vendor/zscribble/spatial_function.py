...
class ModelWeightGatedCRF(torch.nn.Module):
    """
    ...
    def forward(
        ...
        spatial_weight = product_kernel_x_y_hat * y_hat_softmax
        ...
        for i, desc in enumerate(kernels_desc):
            ...
            for modality, sigma in desc.items():
                if modality == 'weight':
                    continue
                if modality == 'xy':
                    feature = ModelWeightGatedCRF._get_mesh(
                    ...
                    feature = sample
                    feature = ModelWeightGatedCRF._downsample(
            ...
            kernel = weight * \
                ModelWeightGatedCRF._create_kernels_from_features(
    ...
    def _create_kernels_from_features(features, radius):
        ...
        kernels = ModelWeightGatedCRF._unfold(features, radius)
