...
def main():
    ...
    for task in tasks_for(args.scenario):
        ...
        if args.base_root:
            base=np.load(previous,allow_pickle=False)["annotations"]; annotations,stats=(generate_wsl_style_from_baseline(labels,base,task.label_shift if args.scenario=="class" else 0,args.seed,args.foreground_area_multiplier,args.background_area_multiplier) if args.style=="wsl4mis" else scale_existing(labels,base,task.label_shift if args.scenario=="class" else 0,args.foreground_area_multiplier,args.background_area_multiplier)); baseline="persisted_v2_snapshot"
        else:
